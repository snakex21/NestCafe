...
pnpm -F @nestcafe/web test                    # Run all web Vitest tests
pnpm -F @nestcafe/web test:unit               # Web unit tests only
pnpm -F @nestcafe/web test:integration        # Web integration tests only

# Testing (desktop workspace — main process + preload tests)
pnpm -F @nestcafe/desktop test                # Run all desktop Vitest tests
pnpm -F @nestcafe/desktop test:unit           # Desktop unit tests only
pnpm -F @nestcafe/desktop test:integration    # Desktop integration tests only
pnpm -F @nestcafe/desktop test:e2e            # Docker-based E2E tests
pnpm -F @nestcafe/desktop test:e2e:native     # Native Playwright E2E tests (serial, Electron requirement)

# Testing (agent-core)
pnpm -F @nestcafe_ai/agent-core test          # Run agent-core Vitest tests
...
pnpm -F @nestcafe/web test

# After changing desktop main process or preload code
pnpm -F @nestcafe/desktop test

# After changing agent-core code
pnpm -F @nestcafe_ai/agent-core test

# Full verification before PR
pnpm lint && pnpm format:check && pnpm -F @nestcafe/web test && pnpm -F @nestcafe/desktop test && pnpm -F @nestcafe_ai/agent-core test
```
...
- **Do NOT add root-level test scripts** — tests are workspace-scoped (`-F @nestcafe/web`, `-F @nestcafe/desktop`, or `-F @nestcafe_ai/agent-core`)
...
- `@nestcafe_ai/agent-core` — Core business logic, types, storage, MCP tools (ESM, internal workspace package)
- `@nestcafe/web` — Standalone React UI (Vite + React Router + Zustand)
- `@nestcafe/desktop` — Thin Electron shell (main process + preload), loads web's build output

## Code Conventions

- TypeScript everywhere (no JS for app logic)
- **ESM package**: `@nestcafe_ai/agent-core` uses `"type": "module"` — all imports MUST use `.js` extensions
...
- IPC handlers in `apps/desktop/src/main/ipc/handlers.ts` must match `window.nestcafe` API in preload
...
3. Add the typed wrapper in `apps/web/src/client/lib/nestcafe.ts`
...
4. Run `pnpm -F @nestcafe_ai/agent-core test`
...
```typescript
"@/*"                              → "src/client/*"
"@nestcafe_ai/agent-core"        → "../../packages/agent-core/src/index.ts"
"@nestcafe_ai/agent-core/*"      → "../../packages/agent-core/src/*"
"@nestcafe_ai/agent-core/common" → "../../packages/agent-core/src/common.ts"
```
...
- `NESTCAFE_BUNDLED_MCP=1` - Bundle MCP tools in packaged build (used in package/release scripts)
