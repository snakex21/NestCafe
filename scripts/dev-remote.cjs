...
if (!url) {
  ...
  console.error('Example: pnpm dev:remote https://nestcafe-app-preview-42.nestcafe.workers.dev');
  process.exit(1);
}

const env = {
  ...process.env,
  NESTCAFE_ROUTER_URL: url,
};

console.log('[dev:remote] Launching Electron → ' + url);

const electronArgs = ['-F', '@nestcafe/desktop', 'dev:remote'];
if (isCheck) {
