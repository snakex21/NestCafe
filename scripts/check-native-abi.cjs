...
const { NODE_VERSION } = require('../layers/desktop/scripts/node-version.cjs');

const workspaces = [
  {
    name: '@nestcafe/daemon',
    dir: path.join(rootDir, 'layers', 'daemon'),
  },
  {
...
function resolveBundledNode() {
  ...
  const platformRoot = path.join(rootDir, 'layers', 'desktop', 'resources', 'nodejs', target);
