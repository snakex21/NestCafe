 ...
 *
 * Ensures layers/daemon/dist/index.js exists before desktop dev starts.
 ...
 * The daemon bundle includes @nestcafe_ai/agent-core code, so agent-core
...
const daemonDir = path.join(rootDir, 'layers', 'daemon');
...
if (needsBuild()) {
  console.log('Building @nestcafe/daemon for dev...');
  try {
    runPnpmSync(['-F', '@nestcafe/daemon', 'build'], {
      cwd: rootDir,
      env: process.env,
      stdio: 'inherit',
    });
    ...
    console.error(`Failed to build @nestcafe/daemon: ${message}`);
    process.exit(1);
  }

  if (!fs.existsSync(daemonDistEntry)) {
    console.error('Failed to produce layers/daemon/dist/index.js');
    process.exit(1);
  }
}

console.log('✓ @nestcafe/daemon build output found');
process.exit(0);
