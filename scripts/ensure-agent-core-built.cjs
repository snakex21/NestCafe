...
const desktopNodeResourcesDir = path.join(rootDir, 'layers', 'desktop', 'resources', 'nodejs');
...
const mcpDistOutputs = [
  ...
  'mcp-tools/wiki-memory/dist/index.mjs',
];
...
if (hostNodeTarget) {
  if (!isHostNodeRuntimeAvailable(hostNodeTarget)) {
    ...
    runPnpm(
      ['-F', '@nestcafe/desktop', 'download:nodejs', `--platform=${hostNodeTarget}`],
...
if (needsAgentCoreBuild) {
  console.log('Missing @nestcafe_ai/agent-core build outputs:');
  ...
  runPnpm(['-F', '@nestcafe_ai/agent-core', 'build'], 'Building @nestcafe_ai/agent-core...');

  missingOutputs = [...outputPaths].filter(
    (relativeOutputPath) => !fs.existsSync(path.join(agentCoreDir, relativeOutputPath)),
  );
  if (missingOutputs.length > 0) {
    console.error('Failed to produce required @nestcafe_ai/agent-core outputs:');
    for (const missingOutput of missingOutputs) {
...
if (missingMcpDistOutputs.length > 0) {
  ...
  runPnpm(
    ['-F', '@nestcafe/desktop', 'build:mcp-tools:dev'],
...
console.log('✓ @nestcafe_ai/agent-core and MCP dist outputs found');
process.exit(0);
