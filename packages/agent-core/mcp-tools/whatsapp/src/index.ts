...
const WHATSAPP_API_PORT = process.env.NESTCAFE_WHATSAPP_API_PORT;
if (!WHATSAPP_API_PORT) {
  process.stderr.write('NESTCAFE_WHATSAPP_API_PORT is not set — WhatsApp MCP tool cannot start\n');
  process.exit(1);
}
const WHATSAPP_API_BASE = `http://localhost:${WHATSAPP_API_PORT}`;
const AUTH_TOKEN = process.env.NESTCAFE_DAEMON_AUTH_TOKEN;
