...
console.error('[dev-browser-mcp] NESTCAFE_TASK_ID:', process.env.NESTCAFE_TASK_ID || '(not set)');
...
async function loadToolDebug(): Promise<void> {
  const debugPath = process.env.NESTCAFE_TOOL_DEBUG_PATH;
  if (debugPath) {
    ...
    console.error('[dev-browser-mcp] NESTCAFE_TOOL_DEBUG_PATH not set, tool debug disabled');
  }
}
await loadToolDebug();
  ...
  try {
    ...
    const frameHandler = async (event: { data: string; sessionId: number }) => {
      try {
        ...
        const taskId = process.env.NESTCAFE_TASK_ID || 'default';
        console.log(
          JSON.stringify({
            type: 'browser-frame',
            taskId,
