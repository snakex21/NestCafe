...
function getDataDir(): string {
  ...
  if (process.platform === 'darwin') {
    ...
    return join(homeDir, '.nestcafe', 'dev-browser');
  }
}
...
const NESTCAFE_HTTP_PORT = parseInt(process.env.DEV_BROWSER_PORT || '9224', 10);
const NESTCAFE_CDP_PORT = parseInt(process.env.DEV_BROWSER_CDP_PORT || '9225', 10);

if (!Number.isFinite(NESTCAFE_HTTP_PORT) || NESTCAFE_HTTP_PORT < 1 || NESTCAFE_HTTP_PORT > 65535) {
  throw new Error(
    `Invalid DEV_BROWSER_PORT: ${process.env.DEV_BROWSER_PORT}. Must be a number between 1 and 65535`,
  );
}
if (!Number.isFinite(NESTCAFE_CDP_PORT) || NESTCAFE_CDP_PORT < 1 || NESTCAFE_CDP_PORT > 65535) {
  throw new Error(
...
try {
  const res = await fetch(`http://localhost:${NESTCAFE_HTTP_PORT}`, {
    ...
    if (info.mode === 'extension') {
      ...
      try {
        if (process.platform === 'win32') {
          const output = execSync(`netstat -ano | findstr :${NESTCAFE_HTTP_PORT}`, {
            encoding: 'utf-8',
          });
          ...
          const pid = execSync(`lsof -ti:${NESTCAFE_HTTP_PORT}`, { encoding: 'utf-8' }).trim();
          if (pid) {
            execSync(`kill -9 ${pid}`);
          }
        }
      ...
      console.log(`Launch server already running on port ${NESTCAFE_HTTP_PORT}`);
      process.exit(0);
    }
  }
} catch {
...
try {
  if (process.platform === 'win32') {
    const output = execSync(`netstat -ano | findstr :${NESTCAFE_CDP_PORT}`, {
      encoding: 'utf-8',
    });
    const match = output.match(/LISTENING\s+(\d+)/);
    if (match) {
      const pid = match[1];
      console.log(
        `Cleaning up stale Chrome process on CDP port ${NESTCAFE_CDP_PORT} (PID: ${pid})`,
      );
      execSync(`taskkill /F /PID ${pid}`, { stdio: 'ignore' });
    }
  } else {
    const pid = execSync(`lsof -ti:${NESTCAFE_CDP_PORT}`, { encoding: 'utf-8' }).trim();
    if (pid) {
      console.log(
        `Cleaning up stale Chrome process on CDP port ${NESTCAFE_CDP_PORT} (PID: ${pid})`,
      );
...
async function startServer(retry = false): Promise<void> {
  try {
    const server = await serve({
      port: NESTCAFE_HTTP_PORT,
      cdpPort: NESTCAFE_CDP_PORT,
      headless,
      profileDir,
