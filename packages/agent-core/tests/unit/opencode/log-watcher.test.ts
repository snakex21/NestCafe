...
import { ERROR_PATTERNS } from '../../../src/internal/classes/log-error-patterns.js';

function extractFirstLogError(line: string): Partial<OpenCodeLogError> | null {
...
describe('OpenCodeLogWatcher', () => {
  ...
  describe('log error pattern matching', () => {
    it('does not treat openai-compatible custom provider auth failures as OpenAI OAuth', () => {
      const line =
        'ERROR service=provider providerID=custom-morlrwc6 modelID=MiniMax-M2.7-highspeed package=@ai-sdk/openai-compatible {"name":"AI_APICallError","message":"Incorrect API key provided","statusCode":401}';

      const error = extractFirstLogError(line);
      ...
      expect(error?.providerID).not.toBe('openai');
      expect(error?.isAuthError).not.toBe(true);
    });

    it('does not treat minimax provider auth failures as OpenAI OAuth even with openai-compatible in raw line', () => {
      const line =
        'ERROR service=provider provider=openai-compatible providerID=minimax modelID=MiniMax-M2.5 Incorrect API key provided';
    ...
    it('still treats explicit OpenAI auth failures as OpenAI OAuth errors', () => {
      const line =
        'ERROR service=provider providerID=openai modelID=gpt-5.2 invalid_api_key Incorrect API key';
