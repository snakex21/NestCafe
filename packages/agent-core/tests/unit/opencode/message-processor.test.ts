...
describe('sanitizeAssistantTextForDisplay', () => {
  ...
  it('preserves pure thinking block for the UI thinking renderer', () => {
    expect(sanitizeAssistantTextForDisplay('<thinking>deep thought</thinking>')).toBe(
      '<thinking>deep thought</thinking>',
    );
  });

  it('preserves unclosed think block for streaming display', () => {
    expect(sanitizeAssistantTextForDisplay('<think>partial thought')).toBe(
