...
import { noopRuntime } from '../../../src/opencode/nestcafe-runtime.js';
...
describe('noopRuntime', () => {
  ...
  it('connect throws NESTCAFE_runtime_unavailable', async () => {
    await expect(noopRuntime.connect(dummyDeps)).rejects.toThrow('NESTCAFE_runtime_unavailable');
  });

  it('getUsage throws NESTCAFE_runtime_unavailable', async () => {
    await expect(noopRuntime.getUsage()).rejects.toThrow('NESTCAFE_runtime_unavailable');
  });
