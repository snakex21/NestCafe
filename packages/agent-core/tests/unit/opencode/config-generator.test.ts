...
describe('ConfigGenerator', () => {
  ...
  const requiredMcpDistEntries = [
    ...
    ['wiki-memory', 'dist/index.mjs'],
    ['search-conversations', 'dist/index.mjs'],
  ] as const;
  ...
  describe('NESTCAFE_AGENT_NAME', () => {
    it('should be "accomplish"', () => {
      expect(NESTCAFE_AGENT_NAME).toBe('accomplish');
    });
  });
  ...
  describe('generateConfig', () => {
    ...
    it('should include base providers in enabled list without enabling OpenAI by default', () => {
      const options: ConfigGeneratorOptions = {
        ...baseOptions,
        mcpToolsPath,
        userDataPath,
      ...
      expect(result.config.enabled_providers).not.toContain('openai');
    ...
    it('should set default agent to accomplish', () => {
      ...
      expect(result.config.default_agent).toBe(NESTCAFE_AGENT_NAME);
    });
    ...
    it('should configure agent with correct mode', () => {
      ...
      expect(result.config.agent?.[NESTCAFE_AGENT_NAME]?.mode).toBe('primary');
    });
