...
describe('OpenCodeAdapter text-part buffer (out-of-order handling)', () => {
  ...
  it('forwards native reasoning parts as thinking-tagged text messages', () => {
    const adapter = constructAdapter();
    const emittedMessages: unknown[] = [];
    const emittedReasoning: string[] = [];
    adapter.on('message', (msg) => {
      emittedMessages.push(msg);
    });
    adapter.on('reasoning', (text) => {
      emittedReasoning.push(text);
    });
    ...
    expect(emittedReasoning).toEqual(['I should answer briefly.']);
    expect(emittedMessages).toHaveLength(1);
    const emitted = emittedMessages[0] as {
      type: string;
      part: { messageID: string; text: string };
    };
    expect(emitted.type).toBe('text');
    expect(emitted.part.messageID).toBe('msg_assistant_1:reasoning');
    expect(emitted.part.text).toBe('<thinking>I should answer briefly.</thinking>');
  });
});
