...
describe('OpenCodeAdapter session resume (sessionId reuse)', () => {
  ...
  function buildFakeClient(): {
    ...
    const client = {
      session: {
        ...
        abort: (args: { sessionID: string }) => {
          sessionAbort.calls += 1;
          sessionAbort.lastSessionID = args.sessionID;
          return Promise.resolve();
        },
      },
      event: {
    ...
    return { client, sessionCreate, sessionPrompt, sessionAbort };
  }
  ...
  it('interruptTask aborts the SDK session and emits an interrupted completion', async () => {
    const adapter = new OpenCodeAdapter(
      { platform: 'darwin', isPackaged: false, tempPath: '/tmp' },
      'tsk_interrupt',
    );
    ...
    await runStartTask(adapter, { prompt: 'do long work' }, fake);
    await adapter.interruptTask();

    expect(fake.sessionAbort.calls).toBe(1);
    expect(fake.sessionAbort.lastSessionID).toBe('freshly_created_session_id');
    expect(completeEvents).toEqual([
      { status: 'interrupted', sessionId: 'freshly_created_session_id' },
    ]);
  });
});
