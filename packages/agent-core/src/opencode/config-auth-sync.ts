...
function toOpenCodeAuthId(providerId: string): string | undefined {
  if (providerId === 'custom' || providerId.startsWith('custom:')) {
    return providerId.replace(/[^a-zA-Z0-9_-]/g, '-');
  }

  return AUTH_KEY_MAPPING[providerId];
}
...
export async function syncApiKeysToOpenCodeAuth(
  ...
  for (const [internalId, key] of Object.entries(apiKeys)) {
    const authId = toOpenCodeAuthId(internalId);
    if (!authId) {
      continue;
    }
