...
export interface ConfigGeneratorOptions {
  ...
  /**
   * Pre-formatted folder-indexing summary injected into the system prompt.
   * Contains directory tree (2 levels deep) with file counts per category
   * for all user-selected indexing folders.
   */
  folderIndexingSummary?: string;
}
...
export interface ProviderModelConfig {
  name: string;
  /** OpenCode model capability flag: enables native file/image parts. */
  attachment?: boolean;
  modalities?: {
    input: Array<'text' | 'audio' | 'image' | 'video' | 'pdf'>;
    output: Array<'text' | 'audio' | 'image' | 'video' | 'pdf'>;
  };
  /** Current OpenCode schema name for tool-call support. */
  tool_call?: boolean;
  /** Legacy Accomplish/OpenCode field kept for compatibility with older runtimes. */
  tools?: boolean;
  limit?: {
    context?: number;
    output?: number;
