...
import {
  ...
  NESTCAFE_SYSTEM_PROMPT_TEMPLATE,
...
const NESTCAFE_PERMISSION_POLICY = {
...
function readInlineSkillContent(filePath: string): string {
  try {
    const content = fs.readFileSync(filePath, 'utf8').trim();
    if (content.length <= MAX_INLINE_SKILL_CHARS) {
      return content;
    }
    return `${content.slice(0, MAX_INLINE_SKILL_CHARS)}\n\n[Skill content truncated]`;
  } catch {
...
function syncPermissionPolicyIntoDefaultConfig(configDir: string, activeConfigPath: string): void {
  ...
  if (!maybeConfig.permission || typeof maybeConfig.permission !== 'object') {
    maybeConfig.permission = { ...NESTCAFE_PERMISSION_POLICY };
  } else {
    const permission = maybeConfig.permission as Record<string, unknown>;
    delete permission['*'];
    Object.assign(permission, NESTCAFE_PERMISSION_POLICY);
  }
...
export function generateConfig(options: ConfigGeneratorOptions): GeneratedConfig {
  ...
  let systemPrompt = NESTCAFE_SYSTEM_PROMPT_TEMPLATE.replace(
...
<wiki-memory>
##############################################################################
# KARPATHY WIKI MEMORY — PERSISTENT MARKDOWN MEMORY
##############################################################################

You have persistent wiki memory stored as Markdown files in:
${memoryDir}

Available memory tools:
- \`search_wiki\` — semantic search across all pages with ranked paragraphs.
  Always call this first when starting a task to recall prior context.
- \`get_wiki\` — read a specific page in full.
- \`update_wiki\` — add facts (append with auto-dedup), replace a page, or
  upsert a section. The tool auto-detects duplicates in append mode.
- \`list_wiki\` — see all wiki pages at a glance.
- \`delete_wiki\` — remove an outdated page.

Memory policy (Karpathy pattern):
0. PERSONAL INFO: When the user shares ANY personal information (name,
   preferences, location, habits, etc.), IMMEDIATELY call \`update_wiki\` to
   save it to the \`user-profile\` page — even if no task is in progress.
   Do NOT wait for task completion. This overrides rule #1.
1. AFTER every meaningful action, save durable facts with \`update_wiki\`.
2. BEFORE starting a task, call \`search_wiki\` to recall relevant context.
3. Store STABLE facts only:
   - User preferences, decisions, and workflow habits
   - Project structure: file locations, conventions, environment quirks
   - Provider/API notes: endpoint quirks, model behaviours, rate limits
   - Task outcomes: what worked, what failed, key findings
   - Credential STATUS only (e.g. "OpenAI API key configured"), NEVER the key itself
4. NEVER store: secrets, passwords, API keys, tokens, PII, or private content
   unless the user explicitly asks you to preserve that exact content.
5. Organize pages by topic:
   - \`user-profile\` — user name, preferences, OS, language
   - \`project-decisions\` — architecture choices, rejected alternatives
   - \`provider-notes\` — quirks per provider (MiniMax, Qwen, etc.)
   - \`task-history\` — date-stamped outcomes of completed tasks
   - \`environment\` — paths, tools, installed versions
6. Update pages incrementally — prefer \`mode: append\` with facts.
   The tool skips exact duplicates automatically.

##############################################################################
</wiki-memory>
`;

  // Inject folder-indexing summary if the user has configured it
  if (folderIndexingSummary) {
    systemPrompt += `\n${folderIndexingSummary}\n`;
  }
...
The full skill instructions are inlined below; follow them directly when a matching skill applies.
...
\`\`\`markdown
${readInlineSkillContent(s.filePath)}
\`\`\``,
  )
  .join('\n\n')}
...
`;
  ...
  if (!bundledNodeBinPath) {
    throw new Error(
      '[OpenCode Config] Missing bundled Node.js path; cannot launch MCP tools. ' +
        'Run "pnpm -F @nestcafe/desktop download:nodejs" and rebuild artifacts.',
    );
  }
  ...
  if (mcpServers['noapi-google-search']) {
    systemPrompt += `

<web-search-tools>
##############################################################################
# NO-API WEB SEARCH MCP
##############################################################################

The noapi-google-search MCP server is available. Use it for live internet
research when the user asks for current information, web search, news, images,
maps, trends, Wikipedia, feeds, YouTube transcription, OCR, or page reading.

Prefer these tools over browser automation for search/research tasks:
- google_search — general web search
- google_news — current news search
- google_scholar — papers and citations
- google_images — image search
- google_trends — trends
- visit_page — fetch and extract readable page text
- wikipedia — Wikipedia summaries
- read_document / ocr_image / transcribe_local when relevant

When using web results, cite or mention the source URLs when possible. If a
search fails because of CAPTCHA/rate limits, explain that briefly and try a
more targeted query or visit known source pages directly.
...
`;
  ...
  const config: OpenCodeConfigFile = {
    ...
    default_agent: NESTCAFE_AGENT_NAME,
    ...
    permission: { ...NESTCAFE_PERMISSION_POLICY },
