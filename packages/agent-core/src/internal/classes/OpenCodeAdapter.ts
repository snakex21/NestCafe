...
import type { FileAttachmentInfo, TaskConfig, Task, TaskResult } from '../../common/types/task.js';
...
import { NESTCAFE_AGENT_NAME } from '../../opencode/config-generator.js';
...
export class OpenCodeAdapter extends EventEmitter<OpenCodeAdapterEvents> {
  ...
  private selectedModelFromConfig: { providerID: string; modelID: string } | null = null;

  /** Most recent permission/question request awaiting a caller reply. */
  private pendingRequest: PendingRequest | null = null;

  /** If enabled for this task turn, tool/file permission prompts are allowed immediately. */
  ...
  async startTask(config: TaskConfig): Promise<Task> {
    ...
    this.selectedModelFromConfig = null;
    ...
    if (this.options.onBeforeStart) {
      ...
      if (rawResult && typeof rawResult === 'object' && 'env' in rawResult) {
        ...
        this.selectedModelFromConfig = result.selectedModel ?? null;
      } else {
    ...
    const promptParts = await this.buildPromptParts(config.prompt, config.files);
    ...
    //
    // CRITICAL: pass `agent: NESTCAFE_AGENT_NAME` explicitly.
    ...
    // and silently ignores the entire `nestcafe` agent prompt — which
    ...
    // reach the model reliably — not just via `agent.nestcafe.prompt`,
   ...
   * identity + conversational-bypass rules live in `agent.nestcafe.prompt`
  ...
  private buildModelParam(config: TaskConfig): { providerID: string; modelID: string } | null {
    ...
    return this.selectedModelFromConfig;
  }
  ...
  private async buildPromptParts(
    ...
    const parts: PromptPartInput[] = [{ type: 'text', text: prompt }];
    if (!files || files.length === 0) {
      return parts;
    }

    for (const file of files) {
      const filePart = await this.buildAttachmentFilePart(file);
      if (filePart) {
        parts.push(filePart);
        continue;
      }

      const textPart = this.buildAttachmentTextPart(file);
      if (textPart) {
        parts.push(textPart);
      }
    }
  ...
  private async buildAttachmentFilePart(file: FileAttachmentInfo): Promise<FilePartInput | null> {
    if (!this.shouldSendAsNativeFilePart(file)) {
      return null;
    }
    ...
    try {
      ...
      return {
        type: 'file',
        mime: this.getAttachmentMimeType(file),
        filename: file.name,
        url: `data:${this.getAttachmentMimeType(file)};base64,${bytes.toString('base64')}`,
      };
    } catch (error) {
      log.warn('failed to read attachment for native file part', {
        file: file.path,
  ...
  private shouldSendAsNativeFilePart(file: FileAttachmentInfo): boolean {
    return file.type === 'image' || file.type === 'pdf';
  }

  private buildAttachmentTextPart(file: FileAttachmentInfo): TextPartInput | null {
    if (!file.content || (file.type !== 'text' && file.type !== 'code')) {
      return null;
    }
    ...
    return {
      type: 'text',
      text: [
        `\n\n<attached-file name="${file.name}" path="${file.path}">`,
        file.content,
        '</attached-file>',
      ].join('\n'),
    };
  ...
  private getAttachmentMimeType(file: FileAttachmentInfo): string {
    ...
    const callbacks: CompletionEnforcerCallbacks = {
      onStartContinuation: async (prompt: string) => {
        if (this.currentSessionId && this.client) {
          // Same `agent: NESTCAFE_AGENT_NAME` reason as the initial
          ...
          const runtimeSystem = this.buildWorkspaceInstructionRuntimeBlock();
          const model = this.selectedModelFromConfig;
          this.client.session
            .prompt({
  ...
  private handleSdkEvent(event: OpenCodeSdkEvent): void {
    ...
    switch (event.type) {
      ...
      case 'session.error': {
        ...
        if (err) {
          const msg = this.formatSessionError(err);
  ...
  private handlePermissionAsked(sdkReq: OpenCodeSdkPermissionRequest): void {
    ...
    if (this.autoApprovePermissions) {
      void this.approvePermissionRequest(sdkReq.id, requestId, req);
      return;
    }
    this.emit('permission-request', req);
  }

  private async approvePermissionRequest(
    sdkRequestId: string,
    requestId: string,
    fallbackRequest: PermissionRequest,
  ): Promise<void> {
    if (!this.client) {
      this.emit('permission-request', fallbackRequest);
      return;
    }
    try {
      await this.client.permission.reply(
        { requestID: sdkRequestId, reply: 'once' },
        { throwOnError: true },
      );
      if (this.pendingRequest?.requestId === requestId) {
        this.pendingRequest = null;
      }
      log.info('Auto-approved permission request', {
        taskId: this.currentTaskId,
        requestId,
      });
    } catch (err) {
      log.warn('Auto-approve permission request failed; showing prompt', {
        taskId: this.currentTaskId,
        requestId,
    ...
    if (part.type === 'reasoning') {
      ...
      return {
        ...
        part: {
          id: asAny.id ?? '',
          sessionID: asAny.sessionID ?? '',
          messageID: `${asAny.messageID ?? asAny.id ?? 'reasoning'}:reasoning`,
          type: 'text',
