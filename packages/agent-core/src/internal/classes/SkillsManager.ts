...
export class SkillsManager {
  ...
  private readonly extraSkillsPaths: string[];
  ...
  constructor(options: SkillsManagerOptions) {
    ...
    this.extraSkillsPaths = options.extraSkillsPaths ?? [];
  }
  ...
  async resync(): Promise<Skill[]> {
    ...
    const extraSkills = this.extraSkillsPaths.flatMap((skillsPath) =>
      scanDirectory(skillsPath, 'community'),
    );

    const allFoundSkills = [...bundledSkills, ...userSkills, ...extraSkills];
  ...
  deleteSkill(skillId: string): boolean {
    ...
    const isManagedUserSkill = path.resolve(skillDir).startsWith(path.resolve(this.userSkillsPath));
    if (isManagedUserSkill && fs.existsSync(skillDir)) {
