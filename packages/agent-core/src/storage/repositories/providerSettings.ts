...
const MAX_CUSTOM_ICON_URL_LENGTH = 64 * 1024;
const MAX_AVAILABLE_MODELS = 500;
const MAX_MODEL_FIELD_LENGTH = 512;

function sanitizeCredentialsForStorage(credentials: ProviderCredentials): ProviderCredentials {
  if (credentials.type !== 'custom') {
    return credentials;
  }

  if (!credentials.iconUrl || credentials.iconUrl.length <= MAX_CUSTOM_ICON_URL_LENGTH) {
    return credentials;
  }
...
function sanitizeAvailableModels(
  models: ConnectedProvider['availableModels'],
  ...
  return models
    ...
    .map((m) => ({
      id: m.id.slice(0, MAX_MODEL_FIELD_LENGTH),
      name: m.name.slice(0, MAX_MODEL_FIELD_LENGTH),
...
function rowToProvider(row: ProviderRow): ConnectedProvider {
  const credentials = sanitizeCredentialsForStorage(
    safeParseJsonWithFallback<ProviderCredentials>(row.credentials_data, {
      type: 'api_key',
      keyPrefix: '',
    })!,
  );
  ...
  return {
    ...
    availableModels: (() => {
      ...
      return sanitizeAvailableModels(valid);
    })(),
...
export function setConnectedProvider(providerId: ProviderId, provider: ConnectedProvider): void {
  const db = getDatabase();
  const credentials = sanitizeCredentialsForStorage(provider.credentials);
  const availableModels = sanitizeAvailableModels(provider.availableModels);
  db.prepare(
...
export function removeConnectedProvider(providerId: ProviderId): void {
  ...
  db.transaction(() => {
    ...
    if (providerId === 'nestcafe-ai') {
      db.prepare('DELETE FROM NESTCAFE_ai_credits WHERE id = 1').run();
    }
  })();
}
...
export function getNestCafeAiCredits(): CreditUsage | null {
  const db = getDatabase();
  const row = db.prepare('SELECT credits_json FROM NESTCAFE_ai_credits WHERE id = 1').get() as
    | { credits_json: string }
    | undefined;
  if (!row) return null;
  try {
...
export function saveNestcafeAiCredits(usage: CreditUsage): void {
  const db = getDatabase();
  db.prepare('INSERT OR REPLACE INTO NESTCAFE_ai_credits (id, credits_json) VALUES (1, ?)').run(
    JSON.stringify(usage),
  );
}

export function clearNestcafeAiCredits(): void {
  const db = getDatabase();
  db.prepare('DELETE FROM NESTCAFE_ai_credits WHERE id = 1').run();
}
