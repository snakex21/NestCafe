 ...
 * `workspace-meta{.db,-dev.db}` file and into the main `nestcafe.db`.
 *
 ...
 * `nestcafe.db` moments apart. Both may observe `status=missing` before
...
export function importLegacyWorkspaceMeta(
  ...
  try {
    const runImport = mainDb.transaction((): ImportOutcome => {
      ...
      //    pre-existing FK issues elsewhere in nestcafe.db (e.g. an
