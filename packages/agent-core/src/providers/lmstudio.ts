...
/**
 * Tests connection to an LM Studio server and scans available models.
 *
 * Makes a GET request to /v1/models to verify connectivity and retrieve
 * the model list without sending inference requests that would load a model.
 *
...
export async function testLMStudioConnection(
  ...
  try {
    ...
    if (!result.models || result.models.length === 0) {
      return {
        success: false,
        error: 'No models found in LM Studio. Make sure the LM Studio server is running.',
      };
    }
