...
import type { FolderIndexingConfig } from './folder-indexing.js';
...
/** Parameters for task.rerunFromMessage */
export interface TaskRerunFromMessageParams {
  taskId: string;
  messageId: string;
  content: string;
  /** Original UI message content, used as a fallback when optimistic UI ids differ from persisted ids. */
...
export interface DaemonMethodMap {
  ...
  'task.rerunFromMessage': { params: TaskRerunFromMessageParams; result: Task };
  ...
  'nestcafe-ai.get-usage': { params: undefined; result: CreditUsage };
  'nestcafe-ai.disconnect': { params: undefined; result: void };
  ...
  'provider.getNestCafeAiCredits': { params: undefined; result: CreditUsage | null };
  'provider.saveNestcafeAiCredits': { params: { usage: CreditUsage }; result: void };
  ...
  'memory.listPages': {
    params: undefined;
    result: Array<{ name: string; size: number; lines: number; mtime: string }>;
  };
  'memory.readPage': {
    params: { page: string };
  ...
  'memory.writePage': {
    params: { page: string; content: string };
    result: void;
  };
  'memory.deletePage': {
    params: { page: string };
  ...
  // Folder indexing
  'folderIndexing.getConfig': { params: undefined; result: FolderIndexingConfig };
  'folderIndexing.setConfig': { params: { config: FolderIndexingConfig }; result: void };
  ...
  // AI text completion (no image needed)
  'ai.complete': {
  ...
  // Modules
  'module.list': { params: undefined; result: import('./module.js').ModuleInstance[] };
  'module.get': {
    params: { id: string };
    result: import('./module.js').ModuleInstance | null;
  };
  'module.install': {
    params: { sourcePath: string };
    result: import('./module.js').ModuleInstance;
  };
  'module.enable': { params: { id: string; enabled: boolean }; result: void };
  'module.uninstall': { params: { id: string }; result: void };
  'module.getSettings': {
...
export interface DaemonNotificationMap {
  ...
  // subscribes via `nestcafe.onAuthError`. Added alongside the P1
  ...
  // subscribes via `nestcafe.onBrowserFrame`. ENG-695 / PR #414 —
  ...
  'nestcafe-ai.usage-update': CreditUsage;
