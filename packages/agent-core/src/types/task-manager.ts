 ...
 * - `selectedModel`: explicit SDK model override parsed from the generated
 *   OpenCode config. The SDK prompt path does not reliably inherit the
 *   config-level `model`, so the adapter passes this on every prompt call.
