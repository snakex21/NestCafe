...
export const SUMMARY_PROMPT_EN = `Generate a very short title (3-5 words max) that summarizes this task request.
...
export function getSummaryPrompt(language?: string): string {
  ...
  const langName = LANGUAGE_DISPLAY_NAMES[language];
  if (!langName) {
    return SUMMARY_PROMPT_EN;
  }
  return `Respond in ${langName}. ${SUMMARY_PROMPT_EN}`;
}
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: getSummaryPrompt(language) + prompt,
          },
        ],
      }),
    ...
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          contents: [
            {
              parts: [{ text: getSummaryPrompt(language) + prompt }],
            },
          ],
          generationConfig: {
            maxOutputTokens: 50,
...
export async function callXAI(apiKey: string, prompt: string, language?: string): Promise<string> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 10000);
