 ...
 * @param language UI language preference for title generation (e.g. 'pl', 'fr', 'ru')
...
export async function generateTaskSummary(
  ...
  for (const provider of providers) {
    ...
    try {
      const summary = await callProviderByName(provider, apiKey, prompt, language);
      if (summary) {
...
async function callProviderByName(
  ...
  switch (provider) {
    case 'anthropic':
      return callAnthropic(apiKey, prompt, language);
    case 'openai':
      return callOpenAI(apiKey, prompt, language);
    case 'google':
      return callGoogle(apiKey, prompt, language);
    case 'xai':
      return callXAI(apiKey, prompt, language);
    default:
      return null;
  }
}
