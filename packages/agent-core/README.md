...
This package is internal to the Accomplish monorepo and is consumed via workspace dependencies (for example, `"@nestcafe_ai/agent-core": "workspace:*"`).

## Quick Start

```typescript
import { createStorage, createTaskManager } from '@nestcafe_ai/agent-core';

// Initialize storage (SQLite + encrypted secure storage)
const storage = createStorage({
  databasePath: '/path/to/nestcafe.db',
...
```typescript
import { createTaskManager } from '@nestcafe_ai/agent-core';
```
...
```typescript
import { createStorage } from '@nestcafe_ai/agent-core';
```
...
```typescript
import { createPermissionHandler } from '@nestcafe_ai/agent-core';
```
...
```typescript
import { createLogWriter } from '@nestcafe_ai/agent-core';
```
...
```typescript
import { createSkillsManager } from '@nestcafe_ai/agent-core';
```
...
```typescript
import { createSpeechService } from '@nestcafe_ai/agent-core';
```
...
```typescript
import { generateConfig, type BrowserConfig } from '@nestcafe_ai/agent-core';
