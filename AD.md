...
| -------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------- |
| **Web UI**           | React frontend — task launcher, execution view, settings, history                         | `apps/web`                          | `stores/taskStore.ts`, `lib/nestcafe.ts`        |
...
| ------------------ | ----------------------------- | ---------------- | -------------------------------------- | ---------------------------------------------- |
| Tasks              | SQLite (`nestcafe.db`)      | Storage Layer    | Create -> Execute -> Complete/Cancel   | Write-heavy during execution, read for history |
...
accomplish/
├── apps/
│   ├── web/                        # @nestcafe/web — Standalone React UI
│   │   ├── src/client/
│   │   │   ├── components/         # UI components (shadcn/ui + custom)
│   │   │   ├── stores/             # Zustand state management
│   │   │   ├── pages/              # Route pages
│   │   │   ├── hooks/              # Custom React hooks
│   │   │   ├── lib/                # Utilities + nestcafe.ts IPC wrapper
│   │   │   └── i18n/               # Internationalization
│   │   └── __tests__/              # Unit + integration tests (Vitest, jsdom)
│   ├── desktop/                    # @nestcafe/desktop — Electron shell
│   │   ├── src/main/
...
│   └── daemon/                     # @nestcafe/daemon — Background process (in-dev)
│       └── src/                    # Task/storage/permission services
├── packages/
│   └── agent-core/                 # @nestcafe_ai/agent-core — Core logic (ESM)
│       └── src/
...
```mermaid
graph LR
    Web["@nestcafe/web<br/>(React UI)"]
    Desktop["@nestcafe/desktop<br/>(Electron Shell)"]
    Daemon["@nestcafe/daemon<br/>(Background Process)"]
    Core["@nestcafe_ai/agent-core<br/>(Core Logic, ESM)"]
...
- Path aliases (`@/*`, `@main/*`, `@nestcafe_ai/agent-core`) enforce clean import boundaries
...
graph TB
    subgraph "User's Machine"
        ...
        subgraph "User Data Directory"
            SQLiteDB["nestcafe.db<br/>(SQLite — all persisted state:<br/>tasks, workspaces, workspace_meta,<br/>knowledge_notes, settings, etc.)"]
