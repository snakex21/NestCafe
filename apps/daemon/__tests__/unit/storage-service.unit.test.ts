 ...
 * (workspace tables now live in `nestcafe.db`), so this test now asserts
 ...
 * from `@nestcafe_ai/agent-core` and assert the shape + call order. The
...
describe('StorageService bootstrap — consolidated workspace-meta', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.NESTCAFE_IS_PACKAGED;
  ...
  it('passes both databasePath and legacyMetaDbPath to createStorage in dev mode', () => {
    ...
    expect(opts.databasePath).toBe(join(dataDir, 'nestcafe-dev.db'));
    expect(opts.legacyMetaDbPath).toBe(join(dataDir, 'workspace-meta-dev.db'));
  });

  it('uses packaged file names when NESTCAFE_IS_PACKAGED=1', () => {
    process.env.NESTCAFE_IS_PACKAGED = '1';
    const svc = new StorageService();
    svc.initialize(dataDir);

    const opts = createStorageSpy.mock.calls[0][0] as Record<string, unknown>;
    expect(opts.databasePath).toBe(join(dataDir, 'nestcafe.db'));
