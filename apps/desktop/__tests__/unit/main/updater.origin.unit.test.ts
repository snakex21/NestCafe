...
describe('isSameApex', () => {
  it('accepts same-apex subdomain (typical accomplish layout)', () => {
    ...
    expect(isSameApex('https://downloads.nestcafe.ai/x/a.exe', 'https://d.nestcafe.ai')).toBe(
      true,
    );
  });
  ...
  it('rejects different apex (attacker-controlled)', () => {
    expect(isSameApex('https://evil.example.com/malware.exe', 'https://d.nestcafe.ai')).toBe(
      false,
    );
  });
  ...
  it('rejects HTTPS → HTTP downgrade even on same apex', () => {
    ...
    expect(isSameApex('http://downloads.nestcafe.ai/a.exe', 'https://d.nestcafe.ai')).toBe(
      false,
    );
  });
  ...
  it('accepts same apex + same scheme (HTTPS)', () => {
    expect(isSameApex('https://downloads.nestcafe.ai/a.exe', 'https://d.nestcafe.ai')).toBe(
      true,
    );
  });
...
describe('isTrustedManifestPath', () => {
  it('accepts relative manifest paths', () => {
    expect(
      isTrustedManifestPath('downloads/1.0.0/app.zip', 'https://downloads.nestcafe.ai'),
    ).toBe(true);
  });
