...
const MANAGED_ENV_KEYS = [
  ...
  'GA_MEASUREMENT_ID',
  'SENTRY_DSN',
  'NESTCAFE_GATEWAY_URL',
  'NESTCAFE_BUILD_ID',
  'NESTCAFE_UPDATER_URL',
  'APP_ROOT',
] as const;
...
describe('loadBuildConfig() — build.env and process.env resolution', () => {
  ...
  describe('only process.env set (OSS dev configuring their own SaaS)', () => {
    ...
    it('reads gateway URL from process.env (enables Free mode in dev)', async () => {
      process.env.NESTCAFE_GATEWAY_URL = 'https://dev.gateway.example.com';
      const cfg = await loadFresh();
    ...
    it('reads values from build.env when no process.env fallback exists', async () => {
      fs.writeFileSync(
        ...
        [
          ...
          'NESTCAFE_GATEWAY_URL=https://gateway.nestcafe.ai',
        ].join('\n'),
      );
      ...
      expect(cfg.accomplishGatewayUrl).toBe('https://gateway.nestcafe.ai');
    });
  });
  ...
  describe('getBuildId() fallback chain', () => {
    it('prefers build.env NESTCAFE_BUILD_ID when present', async () => {
      fs.writeFileSync(buildEnvPath, 'NESTCAFE_BUILD_ID=ci-build-abc123\n');
      await loadFresh();
      const { getBuildId } = await import('@main/config/build-config');
      expect(getBuildId()).toBe('ci-build-abc123');
    });

    it('falls back to process.env NESTCAFE_BUILD_ID when build.env lacks it', async () => {
      // build.env exists but has no BUILD_ID; env var should be picked up.
      fs.writeFileSync(buildEnvPath, 'MIXPANEL_TOKEN=anything\n');
      process.env.NESTCAFE_BUILD_ID = 'env-build-xyz789';
      await loadFresh();
      const { getBuildId } = await import('@main/config/build-config');
  ...
  describe('auto-updater gate (NESTCAFE_UPDATER_URL)', () => {
    ...
    it('dev mode + process.env set: isAutoUpdaterEnabled() returns true (dev opt-in)', async () => {
      process.env.NESTCAFE_UPDATER_URL = 'https://d.nestcafe.ai';
      mockApp.isPackaged = false;
      const cfg = await loadFresh();
      expect(cfg.accomplishUpdaterUrl).toBe('https://d.nestcafe.ai');
      const { isAutoUpdaterEnabled } = await import('@main/config/build-config');
    ...
    it('packaged mode + only process.env set: isAutoUpdaterEnabled() returns false (security invariant)', async () => {
      // Packaged OSS binaries must NOT honor a runtime NESTCAFE_UPDATER_URL env var —
      // the updater spawns an installer and a rogue URL would be RCE.
      process.env.NESTCAFE_UPDATER_URL = 'https://evil.example.com';
    ...
    it('packaged mode + build.env set: isAutoUpdaterEnabled() returns true (Free CI build)', async () => {
      fs.writeFileSync(buildEnvPath, 'NESTCAFE_UPDATER_URL=https://d.nestcafe.ai\n');
      mockApp.isPackaged = true;
      (process as { resourcesPath?: string }).resourcesPath = tempDir;
      const cfg = await loadFresh();
      expect(cfg.accomplishUpdaterUrl).toBe('https://d.nestcafe.ai');
    ...
    it('both sources set (dev): build.env takes precedence', async () => {
      fs.writeFileSync(buildEnvPath, 'NESTCAFE_UPDATER_URL=https://from-build-env.example.com\n');
      process.env.NESTCAFE_UPDATER_URL = 'https://from-process-env.example.com';
      mockApp.isPackaged = false;
      const cfg = await loadFresh();
    ...
    it('empty build.env value (dev): falls back to process.env', async () => {
      fs.writeFileSync(buildEnvPath, 'NESTCAFE_UPDATER_URL=\n');
      process.env.NESTCAFE_UPDATER_URL = 'https://env-wins.example.com';
      mockApp.isPackaged = false;
      const cfg = await loadFresh();
