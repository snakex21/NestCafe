...
// value imports from `@nestcafe_ai/agent-core` → `@nestcafe_ai/agent-core/desktop-main`.
...
vi.mock('@nestcafe_ai/agent-core/desktop-main', async () => {
  return await vi.importMock('@nestcafe_ai/agent-core');
});

// Mock @nestcafe_ai/agent-core - comprehensive mock covering all exports used by handlers.ts
vi.mock('@nestcafe_ai/agent-core', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@nestcafe_ai/agent-core')>();
...
describe('IPC Handlers Integration', () => {
  ...
  describe('Settings Handlers', () => {
    ...
    it('opencode:auth:slack:status should return Slack MCP auth status', async () => {
      const { getSlackMcpOauthStatus } = await import('@nestcafe_ai/agent-core/desktop-main');
  ...
  describe('Task Callbacks and Message Batching', () => {
    ...
    it('task:start should proxy to daemon without calling storage directly', async () => {
      ...
      const { saveTask } = await import('@nestcafe_ai/agent-core');
  ...
  describe('Session Resume with Existing Task', () => {
    ...
    it('session:resume should return task from daemon', async () => {
      ...
      const { updateTaskStatus } = await import('@nestcafe_ai/agent-core');
    ...
    it('session:resume should proxy to daemon without existing task ID', async () => {
      ...
      const { addTaskMessage } = await import('@nestcafe_ai/agent-core');
