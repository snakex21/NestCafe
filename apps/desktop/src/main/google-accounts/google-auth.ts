...
import type { GoogleAccountToken } from '@nestcafe_ai/agent-core/common';
...
export async function startGoogleOAuth(label: string): Promise<{
  ...
  const clientId = getBuildConfig().googleClientId || process.env.GOOGLE_CLIENT_ID || '';
  if (!clientId) {
    getLogCollector().log('ERROR', 'main', 'GOOGLE_CLIENT_ID is not set — OAuth disabled');
    throw new Error(
      'Google OAuth is not configured: missing GOOGLE_CLIENT_ID. Add GOOGLE_CLIENT_ID to apps/desktop/.env or build.env, then restart the app.',
    );
  }
