  ...
  for (;;) {
    try {
      ...
      for (;;) {
        const response = await dialog.showMessageBox({
          type: 'error',
          title: 'NestCafe cannot start',
          message: 'The background service failed to start.',
          detail:
            'NestCafe stores your settings, conversations, and credentials in a background ' +
...
export async function startApp(
  ...
  process.env.NESTCAFE_BUILD_ID = getBuildId();
  ...
  // HuggingFace auto-start + nestcafe-ai cleanup used to run here in the
  ...
  // `nestcafe-ai` under the free tier, the old pre-import cleanup would
  ...
  // is routed here instead.
  {
    try {
      ...
      // Clean up stale nestcafe-ai provider if free mode is no longer
      ...
      try {
        ...
        if (!isFreeMode()) {
          const connected = snap.providers.connectedProviders['nestcafe-ai'];
          if (connected) {
            const client = getDaemonClient();
