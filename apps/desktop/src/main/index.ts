...
async function stopDetachedDaemonForCleanStart(
  ...
  let DaemonClientCtor: typeof import('@nestcafe_ai/agent-core/desktop-main').DaemonClient;
  let createSocketTransport: typeof import('@nestcafe_ai/agent-core/desktop-main').createSocketTransport;
  try {
    const mod = await import('@nestcafe_ai/agent-core/desktop-main');
...
const ROUTER_URL = process.env.NESTCAFE_ROUTER_URL;
...
if (process.platform === 'win32' && !app.isPackaged) {
  app.setAsDefaultProtocolClient('nestcafe', process.execPath, [path.resolve(process.argv[1])]);
} else {
  app.setAsDefaultProtocolClient('nestcafe');
}
