 ...
 *
 * URL comes from build-config (`NESTCAFE_UPDATER_URL`, CI-injected in Free builds
 * or opt-in via .env in dev). Manifest names are hardcoded per the nestcafe-release
 * upload contract — do not parameterize.
 *
 * See: nestcafe-release/scripts/upload-r2-{windows,macos,linux}.sh
 */
...
/**
 * Manifest filenames as published by nestcafe-release. DO NOT parameterize —
