...
async function transcribeWithCohere(audioData: Buffer, mimeType?: string): Promise<
  { success: true; result: { text: string; duration: number; timestamp: number } } | { success: false; error: { code: string; message: string } }
> {
  const apiKey = await getApiKey('cohere');
  if (!apiKey) return { success: false, error: { code: 'MISSING_API_KEY', message: 'Klucz Cohere nie skonfigurowany' } };
  ...
  if (!res.ok) { const err = await res.text().catch(() => ''); return { success: false, error: { code: 'API_ERROR', message: err.slice(0, 200) } }; }
  const data = await res.json() as { text: string };
  return { success: true, result: { text: data.text?.trim() || '', duration: 0, timestamp: Date.now() } };
}
...
export function registerSpeechHandlers(): void {
  ...
  handle('speech:get-config', async () => {
    const key = await getApiKey('elevenlabs');
    return { enabled: Boolean(key && key.trim()), hasApiKey: Boolean(key), apiKeyPrefix: key ? key.substring(0, 8) + '...' : undefined };
  });
  ...
  handle('speech:transcribe', async (
    _event: IpcMainInvokeEvent, audioData: ArrayBuffer, mimeType?: string,
    provider?: 'local-whisper' | 'elevenlabs' | 'cohere', whisperModel?: string,
  ) => {
