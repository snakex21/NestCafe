import type { FileAttachmentInfo } from '@nestcafe_ai/agent-core/desktop-main';
...
export function sanitizeAttachments(attachments: unknown[] | undefined): FileAttachmentInfo[] {
  ...
  for (const item of attachments) {
    ...
    const id = typeof a['id'] === 'string' ? a['id'].slice(0, 256) : undefined;
    ...
    if (!id || !path || !name || !type || size === undefined) {
      continue;
    }
