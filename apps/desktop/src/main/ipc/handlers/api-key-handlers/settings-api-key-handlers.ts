...
import { sanitizeString } from '@nestcafe_ai/agent-core/desktop-main';
import { ALLOWED_API_KEY_PROVIDERS } from '@nestcafe_ai/agent-core/desktop-main';
import {
  storeApiKey,
  deleteApiKey,
  getApiKey,
...
export function registerSettingsApiKeyHandlers(): void {
  ...
  handle(
    ...
    async (_event: IpcMainInvokeEvent, provider: string, key: string, label?: string) => {
      if (!ALLOWED_API_KEY_PROVIDERS.has(provider) && !provider.startsWith('custom:')) {
  ...
  handle('settings:get-api-key', async (_event: IpcMainInvokeEvent, provider: string) => {
    const sanitizedProvider = sanitizeString(provider, 'provider', 128);
    if (
      !ALLOWED_API_KEY_PROVIDERS.has(sanitizedProvider) &&
      !sanitizedProvider.startsWith('custom:')
    ) {
      throw new Error('Unsupported API key provider');
    }

    return await getApiKey(sanitizedProvider);
  });
