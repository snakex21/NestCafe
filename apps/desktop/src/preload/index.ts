...
import type { MessagingConnectionStatus } from '@nestcafe_ai/agent-core/common';
import type {
  ProviderType,
  Skill,
  TodoItem,
...
// Expose the nestcafe API to the renderer
const NestCafeAPI = {
  ...
  pickFiles: (): Promise<import('@nestcafe_ai/agent-core/common').FileAttachmentInfo[]> =>
    ipcRenderer.invoke('files:pick'),
  processDroppedFiles: (
    paths: string[],
  ): Promise<import('@nestcafe_ai/agent-core/common').FileAttachmentInfo[]> =>
  ...
  NestcafeAiConnect: (): Promise<unknown> => ipcRenderer.invoke('nestcafe-ai:connect'),
  NestcafeAiEnsureReady: (): Promise<unknown> => ipcRenderer.invoke('nestcafe-ai:ensure-ready'),
  NestcafeAiDisconnect: (): Promise<void> => ipcRenderer.invoke('nestcafe-ai:disconnect'),
  NestcafeAiGetUsage: (): Promise<unknown> => ipcRenderer.invoke('nestcafe-ai:get-usage'),
  NestcafeAiGetStatus: (): Promise<{ connected: boolean }> =>
    ipcRenderer.invoke('nestcafe-ai:get-status'),
  onNestcafeAiUsageUpdate: (callback: (usage: unknown) => void) => {
    const listener = (_: unknown, usage: unknown) => callback(usage);
    ipcRenderer.on('nestcafe-ai:usage-updated', listener);
  ...
  folderIndexing: {
    scanFolders: (
      folderPaths: string[],
    ): Promise<import('@nestcafe_ai/agent-core/common').FolderScanResult[]> =>
      ipcRenderer.invoke('folder-indexing:scan-folders', folderPaths),

    getConfig: (): Promise<import('@nestcafe_ai/agent-core/common').FolderIndexingConfig> =>
      ipcRenderer.invoke('folder-indexing:get-config'),

    setConfig: (
      config: import('@nestcafe_ai/agent-core/common').FolderIndexingConfig,
    ): Promise<void> => ipcRenderer.invoke('folder-indexing:set-config', config),
