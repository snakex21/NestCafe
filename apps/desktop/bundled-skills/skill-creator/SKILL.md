...
## Saving Skills in NestCafe

**IMPORTANT:** When creating skills in NestCafe, you can ONLY create "custom" skills. You CANNOT create "official" skills - those are bundled with the app and managed by the NestCafe team.
...
Skills must be saved to the NestCafe user data directory under a `skills` folder:

**macOS:** `~/Library/Application Support/NestCafe/skills/<skill-name>/SKILL.md`
**Windows:** `%APPDATA%\NestCafe\skills\<skill-name>\SKILL.md`
**Linux:** `~/.config/NestCafe/skills/<skill-name>/SKILL.md`
...
1. **Use the base path from the task prompt** when present.
   - If no path is provided, detect the OS and use:
     - macOS: `~/Library/Application Support/NestCafe/skills/`
     - Windows: `%APPDATA%\NestCafe\skills\`
     - Linux: `~/.config/NestCafe/skills/`
...
5. **The skill is automatically detected** - NestCafe scans this directory on startup and syncs new skills to its database. The skill will appear in Settings > Skills as a "Custom" skill.

### Skill Frontmatter Rules

For custom skills in NestCafe:
