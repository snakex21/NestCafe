...
import type { GoogleAccount } from '@nestcafe_ai/agent-core/common';
...
// Build a mock window.nestcafe.gws API
function makeGwsApi(
  overrides: Partial<{
...
describe('useGoogleAccountStore', () => {
  ...
  beforeEach(() => {
    ...
    g.window.nestcafe = {
      ...(g.window.nestcafe ? (g.window.nestcafe as object) : {}),
      gws: gwsApi,
    };
  });
  ...
  describe('removeAccount', () => {
    ...
    it('calls window.nestcafe.gws.removeAccount with the correct id', async () => {
      const { useGoogleAccountStore } = await import('@/stores/googleAccountStore');
