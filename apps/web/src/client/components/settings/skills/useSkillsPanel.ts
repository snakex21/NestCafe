...
import type { Skill } from '@nestcafe_ai/agent-core';
...
export function useSkillsPanel(refreshTrigger?: number): UseSkillsPanelResult {
  ...
  const handleToggle = useCallback(
    async (id: string) => {
      ...
      if (!skill || !window.nestcafe) {
        return;
      }
      try {
        await window.nestcafe.setSkillEnabled(id, !skill.isEnabled);
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        await window.nestcafe.deleteSkill(id);
  ...
  const handleEdit = useCallback(async (filePath: string) => {
    ...
    try {
      await window.nestcafe.openSkillInEditor(filePath);
    } catch (err) {
  ...
  const handleShowInFolder = useCallback(async (filePath: string) => {
    ...
    try {
      await window.nestcafe.showSkillInFolder(filePath);
    } catch (err) {
  ...
  const handleResync = useCallback(async () => {
    if (!window.nestcafe || isResyncing) {
      return;
    }
    setIsResyncing(true);
    try {
      const [updatedSkills] = await Promise.all([
        window.nestcafe.resyncSkills(),
