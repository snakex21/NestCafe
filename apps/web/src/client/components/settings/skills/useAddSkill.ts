...
export function useAddSkill(): UseAddSkillResult {
  ...
  const handleUploadSkill = async (onSkillAdded?: () => void) => {
    if (isLoadingRef.current || !window.nestcafe) {
      return;
    }
    try {
      ...
      setIsLoading(true);
      setUploadError(null);
      const folderPath = await window.nestcafe.pickSkillFolder();
      if (!folderPath) {
        setIsLoading(false);
        return;
      }
      await window.nestcafe.addSkillFromFolder(folderPath);
      onSkillAdded?.();
    } catch (err) {
  ...
  const handleImportFromGitHub = async (onSkillAdded?: () => void) => {
    ...
    if (isLoadingRef.current || !normalizedGitHubUrl || !window.nestcafe) {
      return;
    }
    try {
      isLoadingRef.current = true;
      setIsLoading(true);
      setError(null);
      await window.nestcafe.addSkillFromGitHub(normalizedGitHubUrl);
      setGitHubUrl('');
