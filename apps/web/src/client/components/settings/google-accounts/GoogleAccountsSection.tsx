...
export function GoogleAccountsSection() {
  ...
  const openAuth = async (
    ...
    setConnecting(true);
    try {
      const result = await window.nestcafe?.gws?.startAuth(label);
      if (result?.authUrl && result.authUrl.length > 0) {
        if (window.nestcafe?.openExternal) {
          await window.nestcafe.openExternal(result.authUrl);
        } else {
  ...
  const handleCancelConnecting = async () => {
    ...
    if (pendingAuthState) {
      await window.nestcafe?.gws?.cancelAuth(pendingAuthState);
    }
    setReconnectId(null);
    setPendingAuthState(null);
    setConnecting(false);
