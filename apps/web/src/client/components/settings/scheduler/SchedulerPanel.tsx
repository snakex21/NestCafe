...
import type { ScheduledTask } from '@nestcafe_ai/agent-core/common';
...
export function SchedulerPanel() {
  ...
  const loadSchedules = useCallback(async () => {
    try {
      const result = await nestcafe.listSchedules(activeWorkspaceId ?? undefined);
      setSchedules(result);
    } catch {
      // Daemon may be unavailable
      setSchedules([]);
    }
  }, [nestcafe, activeWorkspaceId]);

  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const [, autoStart] = await Promise.all([loadSchedules(), nestcafe.isAutoStartEnabled()]);
    ...
    load();
  }, [loadSchedules, nestcafe]);

  const handleCreate = async (cron: string, prompt: string) => {
    await nestcafe.createSchedule(cron, prompt, activeWorkspaceId ?? undefined);
    await loadSchedules();
  };

  const handleToggleEnabled = async (id: string, enabled: boolean) => {
    await nestcafe.setScheduleEnabled(id, enabled);
    // Re-fetch to get updated next_run_at (recomputed server-side on enable)
    await loadSchedules();
  };

  const handleDelete = async (id: string) => {
    await nestcafe.deleteSchedule(id);
