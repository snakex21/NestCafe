...
import type { ConnectedProvider, CopilotOAuthCredentials } from '@nestcafe_ai/agent-core';
import { COPILOT_MODELS } from '@nestcafe_ai/agent-core/common';
...
export function useCopilotConnection({
  ...
  useEffect(() => {
    ...
    nestcafe.getCopilotOAuthStatus()
      .then((status) => {
        if (status.connected) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const result = await nestcafe.loginGithubCopilot();
      ...
      if (result.ok) {
        ...
        const poll = async () => {
          for (let i = 0; i < MAX_ATTEMPTS; i++) {
            ...
            const status = await nestcafe.getCopilotOAuthStatus();
            if (status.connected) {
              onConnect(buildCopilotProvider());
              setUserCode(null);
              setVerificationUri(null);
  ...
  const handleDisconnect = async () => {
    try {
      const nestcafe = getNestCafe();
      await nestcafe.logoutGithubCopilot();
    } catch (err) {
