...
import type { ConnectedProvider, VertexProviderCredentials } from '@nestcafe_ai/agent-core';
...
export function useVertexProviderConnect({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await nestcafe.validateVertexCredentials(credentials);
      ...
      await nestcafe.saveVertexCredentials(credentials);
      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await nestcafe.fetchVertexModels(credentialsJson);
