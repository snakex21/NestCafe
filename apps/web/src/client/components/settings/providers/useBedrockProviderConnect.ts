...
export function useBedrockProviderConnect({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await nestcafe.validateBedrockCredentials(credentials);
      ...
      await nestcafe.saveBedrockCredentials(credentials);

      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await nestcafe.fetchBedrockModels(credentialsJson);
