...
import type { ProviderId, ConnectedProvider, OAuthCredentials } from '@nestcafe_ai/agent-core';
...
export function useOAuthSignIn({
  ...
  const handleChatGptSignIn = async () => {
    ...
    try {
      ...
      const result = await nestcafe.loginOpenAiWithChatGpt();
      ...
      const poll = async () => {
        for (let i = 0; i < MAX_ATTEMPTS; i++) {
          ...
          const status = await nestcafe.getOpenAiOauthStatus();
