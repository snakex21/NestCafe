...
import type { ConnectedProvider } from '@nestcafe_ai/agent-core/common';
import type { HuggingFaceLocalCredentials } from '@nestcafe_ai/agent-core/common';
...
export function useHuggingFaceProviderConnect({
  ...
  useEffect(() => {
    ...
    nestcafe.listHuggingFaceModels()
  ...
  useEffect(() => {
    const nestcafe = getNestCafe();
    const unsub = nestcafe.onHuggingFaceDownloadProgress((progress) => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const downloadResult = await nestcafe.downloadHuggingFaceModel(selectedModelId);
      ...
      const serverResult = await nestcafe.startHuggingFaceServer(selectedModelId);
  ...
  const handleDisconnect = async () => {
    try {
      const nestcafe = getNestCafe();
      await nestcafe.stopHuggingFaceServer();
    } catch {
