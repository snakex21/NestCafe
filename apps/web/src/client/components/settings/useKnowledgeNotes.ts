...
import type { KnowledgeNote, KnowledgeNoteType } from '@nestcafe_ai/agent-core';

type AccomplishInstance = ReturnType<typeof getNestCafe>;
...
export function useKnowledgeNotes(
  ...
  const loadNotes = useCallback(async () => {
    ...
    try {
      const loaded = await nestcafe.listKnowledgeNotes(workspaceId);
  ...
  useEffect(() => {
    const requestId = ++activeRequestRef.current;
    nestcafe.listKnowledgeNotes(workspaceId)
      .then((loaded) => {
        if (requestId === activeRequestRef.current && workspaceIdRef.current === workspaceId) {
          setError(null);
          setNotes(loaded);
  ...
  const handleAdd = useCallback(async () => {
    ...
    try {
      await nestcafe.createKnowledgeNote({
        workspaceId,
        type: newType,
  ...
  }, [nestcafe, workspaceId, newType, newContent, loadNotes]);
  ...
  const handleEdit = useCallback(
    async (id: string) => {
      ...
      try {
        const updated = await nestcafe.updateKnowledgeNote(id, workspaceId, {
          type: editType,
    ...
    [nestcafe, workspaceId, editType, editContent, loadNotes, t],
  );

  const handleDelete = useCallback(
    async (id: string) => {
      try {
        const deleted = await nestcafe.deleteKnowledgeNote(id, workspaceId);
        if (workspaceIdRef.current !== workspaceId) {
          return;
        }
        if (!deleted) {
    ...
    [nestcafe, workspaceId, loadNotes, t],
  );
