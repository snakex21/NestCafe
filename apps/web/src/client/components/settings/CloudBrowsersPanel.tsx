...
export function CloudBrowsersPanel() {
  ...
  useEffect(() => {
    nestcafe.getCloudBrowserConfig()
      .then((c) => {
        if (c) {
          setConfig(c);
        }
      ...
      .catch((err) => {
        const message = err instanceof Error ? err.message : t('cloudBrowsers.loadFailed');
        setSaveError(message);
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps -- nestcafe is a stable singleton
  }, []);
  ...
  const saveConfig = useCallback(
    async (newConfig: CloudBrowserConfig) => {
      ...
      setSaveError(null);
      try {
        await nestcafe.setCloudBrowserConfig(newConfig);
        setConfig(newConfig);
      } catch (err) {
        const message = err instanceof Error ? err.message : t('cloudBrowsers.saveFailed');
        setSaveError(message);
      } finally {
        setSaving(false);
      }
  ...
  const activeProviderLabel =
    config.activeProvider === null
      ? t('cloudBrowsers.localBrowser')
      : t('cloudBrowsers.usingProvider', {
          provider:
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-card p-5">
        <div className="font-medium text-foreground">{t('tabs.browsers')}</div>
        <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
          {t('cloudBrowsers.description')}
        </p>
