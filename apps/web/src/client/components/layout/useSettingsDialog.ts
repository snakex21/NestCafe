...
import { hasAnyReadyProvider, isProviderReady } from '@nestcafe_ai/agent-core/common';
...
export function useSettingsDialog({
  ...
  useEffect(() => {
    ...
    refetch();
    nestcafe.getDebugMode().then(setDebugModeState);
    nestcafe.getNotificationsEnabled().then(setNotificationsEnabledState);
    nestcafe.getVersion().then(setAppVersion);
  }, [open, refetch, nestcafe]);
  ...
  const handleUpdateProvider = useCallback(
    async (provider: ConnectedProvider) => {
      await connectProvider(provider.providerId, provider);
      if (settings?.activeProviderId === provider.providerId && !isProviderReady(provider)) {
        ...
        await setActiveProvider(readyId ?? null);
      }
    },
    [connectProvider, settings, setActiveProvider],
  );
  ...
  const handleDebugToggle = useCallback(async () => {
    ...
    await nestcafe.setDebugMode(newValue);
    setDebugModeState(newValue);
  }, [debugMode, nestcafe]);

  const handleNotificationsToggle = useCallback(async () => {
    const newValue = !notificationsEnabled;
    await nestcafe.setNotificationsEnabled(newValue);
    setNotificationsEnabledState(newValue);
  }, [notificationsEnabled, nestcafe]);
  ...
  const handleAddCustomProvider = useCallback(async (): Promise<ProviderId> => {
    ...
    const customCount = customProviderIds.length;
    const displayName =
      customCount === 0 ? 'Custom Provider' : `Custom Provider ${customCount + 1}`;
    const newProviderId = `custom:${Date.now().toString(36)}` as ProviderId;
    ...
    await connectProvider(newProviderId, newProvider);
    return newProviderId;
  }, [settings?.connectedProviders, connectProvider]);
