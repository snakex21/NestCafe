...
import type { GoogleAccount, GoogleAccountStatus } from '@nestcafe_ai/agent-core/common';
...
export const useGoogleAccountStore = create<GoogleAccountStore>((set) => ({
  ...
  fetchAccounts: async () => {
    ...
    try {
      const accounts = await window.nestcafe?.gws?.listAccounts();
      if (!accounts) {
  ...
  removeAccount: async (id: string) => {
    try {
      await window.nestcafe?.gws?.removeAccount(id);
      set((state) => ({
  ...
  updateLabel: async (id: string, label: string) => {
    try {
      await window.nestcafe?.gws?.updateLabel(id, label);
      set((state) => ({
...
export function initGoogleAccountListener(): () => void {
  ...
  const unsubscribeStatus = window.nestcafe?.gws?.onStatusChanged((id, status) => {
  ...
  const unsubscribeAuthError = window.nestcafe?.gws?.onAuthError(({ message }) => {
