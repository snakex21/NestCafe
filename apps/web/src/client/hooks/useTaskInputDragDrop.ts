...
import type { FileAttachmentInfo } from '@nestcafe_ai/agent-core';
...
export function useTaskInputDragDrop({
  ...
  const handleDrop = useCallback(
        ...
        const filePaths = Array.from(e.dataTransfer.files)
          .map((file) => {
            if (nestcafe.getFilePath) {
              try {
                return nestcafe.getFilePath(file);
              } catch {
                return undefined;
              }
            }
          ...
          .filter(
            (filePath): filePath is string => typeof filePath === 'string' && filePath !== '',
          );

        if (filePaths.length > 0) {
          void nestcafe.processDroppedFiles(filePaths)
            .then((processed) => {
              if (processed.length === 0 || !onAttachmentsChange) {
                return;
              }
              const remaining = MAX_FILES - attachments.length;
              if (remaining <= 0) {
                setAttachmentError(t('plusMenu.tooManyFiles', { max: MAX_FILES }));
                return;
              }
              onAttachmentsChange([...attachments, ...processed.slice(0, remaining)]);
            })
            .catch(() => {
              setAttachmentError(t('plusMenu.attachFailed'));
            });
          return;
        }
      }
    ...
    [isInputDisabled, addFiles, onAttachmentsChange, attachments, t],
  );
