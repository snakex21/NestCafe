...
packages/agent-core/   # Core business logic, types, storage, MCP tools (@nestcafe_ai/agent-core, ESM)
```
...
pnpm -F @nestcafe/web test:unit            # Web unit tests
pnpm -F @nestcafe/web test:integration     # Web integration tests
pnpm -F @nestcafe/desktop test:unit        # Desktop main-process unit tests
pnpm -F @nestcafe/desktop test:integration # Desktop integration tests
pnpm -F @nestcafe/desktop test:e2e:native  # Playwright E2E tests (serial, Electron)
pnpm -F @nestcafe_ai/agent-core test       # Agent-core tests
pnpm -F @nestcafe/daemon test              # Daemon tests

# Run a single test file
pnpm -F @nestcafe/desktop vitest run --config vitest.unit.config.ts path/to/file.unit.test.ts
pnpm -F @nestcafe/web vitest run --config vitest.unit.config.ts path/to/file.unit.test.ts
...
# NESTCAFE_BUNDLED_MCP=1 — bundle MCP tools (used in package/release scripts only)
```
...
3. Typed wrapper in `apps/web/src/client/lib/nestcafe.ts`
...
- DB: `nestcafe.db` (prod) / `nestcafe-dev.db` (dev), in Electron user-data directory
...
- **No root-level test scripts** — always use `-F @nestcafe/web`, `-F @nestcafe/desktop`, or `-F @nestcafe_ai/agent-core`
...
pnpm -F @nestcafe/web test:unit          # if apps/web changed
pnpm -F @nestcafe/desktop test:unit      # if apps/desktop changed
pnpm -F @nestcafe_ai/agent-core test     # if packages/agent-core changed
pnpm -F @nestcafe/daemon test            # if apps/daemon changed
```
