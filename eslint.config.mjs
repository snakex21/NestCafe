...
export default tseslint.config(
  ...
  //
  // Electron main must NOT value-import from root `@nestcafe_ai/agent-core`
  // — the root barrel re-exports `createStorage`, which transitively pulls
  // `better-sqlite3`. Use `@nestcafe_ai/agent-core/desktop-main` for values,
  // `@nestcafe_ai/agent-core/common` for pure types that aren't already
  ...
  // is empty (after Milestone 5), this rule becomes the end-state invariant.
  {
    ...
    ignores: [
      ...
      // force: every Electron-main value import from `@nestcafe_ai/agent-core`
      // is a violation. Type-only imports are still allowed (see
      // `allowTypeImports: true` on the rule).
      //
      // Migration history:
      //   M1:   new `@nestcafe_ai/agent-core/desktop-main` subpath introduced,
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          paths: [
            {
              name: '@nestcafe_ai/agent-core',
              message:
                'Use `@nestcafe_ai/agent-core/desktop-main` for value imports, or `@nestcafe_ai/agent-core/common` for pure types. Root is DB-bound (pulls better-sqlite3) and must not be value-imported from Electron main.',
              allowTypeImports: true,
            },
            {
              // M6 review finding P2.D: the root-barrel ban above catches
              // `import X from '@nestcafe_ai/agent-core'`, but agent-core's
              // tsconfig wildcard exposes every concrete submodule too
              // (`@nestcafe_ai/agent-core/*`). A future main/preload file
          ...
          patterns: [
            {
              ...
              group: [
                '@nestcafe_ai/agent-core/storage',
                '@nestcafe_ai/agent-core/storage/*',
                '@nestcafe_ai/agent-core/factories',
                '@nestcafe_ai/agent-core/factories/storage',
                '@nestcafe_ai/agent-core/internal',
                '@nestcafe_ai/agent-core/internal/*',
              ],
              message:
                'Deep imports into agent-core storage/factories/internal are banned from Electron main. Every one of these modules reaches `better-sqlite3`. Route through `@nestcafe_ai/agent-core/desktop-main` (values) or `@nestcafe_ai/agent-core/common` (pure types).',
            },
          ],
        },
      ],
