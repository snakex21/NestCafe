...
function reloadRendererAfterCrash(window: BrowserWindow): void {
  setTimeout(() => {
...
export function createMainWindow(opts: {
  ...
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    logMain('ERROR', `[Main] Renderer process gone: ${details.reason} (${details.exitCode})`);
    reloadRendererAfterCrash(mainWindow);
  });

  mainWindow.webContents.on('unresponsive', () => {
    logMain('WARN', '[Main] Renderer became unresponsive');
  });

  mainWindow.webContents.on('responsive', () => {
    logMain('INFO', '[Main] Renderer became responsive');
  });
  ...
  mainWindow.webContents.on(
    ...
    (_event, errorCode, errorDescription, validatedURL) => {
      logMain(
        'ERROR',
        `[Main] Renderer failed to load ${validatedURL}: ${errorCode} ${errorDescription}`,
      );
    },
  );
  ...
  mainWindow.webContents.session.webRequest.onHeadersReceived((details, callback) => {
    const scriptSrc = app.isPackaged ? "'self' blob:" : "'self' 'unsafe-inline' blob:";
