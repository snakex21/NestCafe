 ...
 *
 * Manifests may carry absolute download URLs (e.g. `https://downloads.nestcafe.ai/...`)
 * on a different subdomain than the manifest host (`https://d.nestcafe.ai/...`), so a
 * strict same-origin check is too tight. Instead we accept any URL whose hostname shares
 * the feed URL's apex (last two labels) — covers `downloads.nestcafe.ai` vs
 * `d.nestcafe.ai` without accepting `evil.example.com`.
 *
 ...
 * "same apex" and accept `bar.co.uk` (unrelated customer). Fine for `nestcafe.ai`, but
...
export function isSameApex(candidate: string, reference: string): boolean {
  ...
  // `http://downloads.nestcafe.ai/...` would otherwise hand the user a
...
function getApex(hostname: string): string {
  ...
  if (labels.length <= 2) {
    // 'localhost' or a bare apex like 'nestcafe.ai' — use as-is.
    return hostname;
  }
