...
import type { DaemonClient } from '@nestcafe_ai/agent-core/desktop-main';
...
>();
import { createSocketTransport } from '@nestcafe_ai/agent-core/desktop-main';
import {
  ensureDaemonRunning,
  onReconnect,
...
export async function bootstrapDaemon(): Promise<DaemonClient> {
  ...
  // and reconnections to already-running daemons. Only when debug mode is on.
  try {
    ...
    // Settings read failed — default to silent (logs remain in file)
  }
...
function registerNotificationHandlers(
  ...
  // subscribes via `nestcafe.onAuthError` in preload.
  ...
  // subscribes via `nestcafe.onBrowserFrame` in preload.
  client.onNotification('browser.frame', (data) => {
    forward('browser:frame', data);
  });

  // Step-finish token usage — forwarded to renderer for context-window indicator
  client.onNotification('task.stepFinish', (data) => {
    forward('task:step-finish', data);
  });

  // Accomplish AI credit usage updates (proxy → daemon → Electron → renderer)
  client.onNotification('nestcafe-ai.usage-update', (data) => {
    forward('nestcafe-ai:usage-updated', data);
  });
