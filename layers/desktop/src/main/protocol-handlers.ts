/**
 * Protocol URL handler helpers for `nestcafe://` deep-link callbacks.
 *
...
function dispatchProtocolUrl(win: BrowserWindow, url: string): void {
  if (url.startsWith('nestcafe://callback/mcp')) {
    win.webContents.send('auth:mcp-callback', url);
  } else if (url.startsWith('nestcafe://callback')) {
...
export function handleProtocolUrlFromArgs(getMainWindow: WindowGetter): void {
  ...
  const protocolUrl = process.argv.find((arg) => arg.startsWith('nestcafe://'));
  if (!protocolUrl) {
    return;
  }
...
export function handleSecondInstanceProtocolUrl(
  ...
  const protocolUrl = commandLine.find((arg) => arg.startsWith('nestcafe://'));
  if (protocolUrl) {
    enqueueProtocolUrl(protocolUrl, getMainWindow);
  }
}
