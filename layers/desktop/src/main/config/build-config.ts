...
export function loadBuildConfig(): BuildConfig {
  ...
  const parsed = buildConfigSchema.safeParse({
    ...
    accomplishGatewayUrl: firstNonEmpty(raw.NESTCAFE_GATEWAY_URL, process.env.NESTCAFE_GATEWAY_URL),
    googleClientId: firstNonEmpty(raw.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_ID),
    buildId: firstNonEmpty(raw.NESTCAFE_BUILD_ID, process.env.NESTCAFE_BUILD_ID),
    ...
    accomplishUpdaterUrl: firstNonEmpty(
      raw.NESTCAFE_UPDATER_URL,
      app.isPackaged ? undefined : process.env.NESTCAFE_UPDATER_URL,
    ),
  });
 ...
 *
 * Priority:
 * 1. Packaged builds: NESTCAFE_BUILD_ID from build.env (injected by release pipeline)
