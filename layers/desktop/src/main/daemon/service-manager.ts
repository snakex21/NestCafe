...
const LAUNCH_AGENT_LABEL = 'ai.nestcafe.daemon';
...
function getLaunchAgentContent(): string {
  ...
  // Electron's environment, so NESTCAFE_APP_PATH would otherwise be missing.
  const envDict = ['  <key>EnvironmentVariables</key>', '  <dict>'];
  if (app.isPackaged) {
    envDict.push('    <key>NESTCAFE_IS_PACKAGED</key><string>1</string>');
  } else {
  ...
  envDict.push(
    `    <key>NESTCAFE_RESOURCES_PATH</key><string>${app.isPackaged ? process.resourcesPath : `${app.getAppPath()}/resources`}</string>`,
    `    <key>NESTCAFE_APP_PATH</key><string>${app.getAppPath()}</string>`,
    '  </dict>',
  );
  lines.push(...envDict);
...
const SYSTEMD_SERVICE_NAME = 'nestcafe-daemon.service';
...
function getSystemdServiceContent(): string {
  ...
  const lines = [
    '[Unit]',
    'Description=NestCafe AI Daemon',
  ...
  if (app.isPackaged) {
    lines.push(
      `Environment=NESTCAFE_IS_PACKAGED=1`,
      `Environment=NESTCAFE_RESOURCES_PATH=${process.resourcesPath}`,
      `Environment=NESTCAFE_APP_PATH=${app.getAppPath()}`,
    );
  }
