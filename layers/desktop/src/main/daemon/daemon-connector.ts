...
import { getPidFilePath } from '@nestcafe_ai/agent-core/desktop-main';
 ...
 *      `pnpm -F @nestcafe/desktop download:nodejs` to have been run;
...
export function spawnDaemon(dataDir: string): void {
  ...
  // or system Node in dev) — never Electron-as-Node. The daemon's
  // `better-sqlite3` is built against Node's ABI, not Electron's.
  ...
  const daemonEnv: Record<string, string | undefined> = {
    ...
    NESTCAFE_BUILD_ID: getBuildId(),
  };
  daemonEnv.NESTCAFE_MEMORY_DIR = app.isPackaged
    ? path.join(path.dirname(process.execPath), 'memory')
    : path.resolve(app.getAppPath(), '..', '..', 'memory');
  ...
  if (bc.googleClientId) {
    daemonEnv.GOOGLE_CLIENT_ID = bc.googleClientId;
  }
  if (app.isPackaged) {
    daemonEnv.NESTCAFE_IS_PACKAGED = '1';
    daemonEnv.NESTCAFE_RESOURCES_PATH = process.resourcesPath;
    daemonEnv.NESTCAFE_APP_PATH = app.getAppPath();
  } else {
    // Dev mode: pass desktop app path so daemon can find bundled Node.js
    // and other resources relative to the desktop workspace
    daemonEnv.NESTCAFE_APP_PATH = app.getAppPath();
    daemonEnv.NESTCAFE_RESOURCES_PATH = path.join(app.getAppPath(), 'resources');
  }
