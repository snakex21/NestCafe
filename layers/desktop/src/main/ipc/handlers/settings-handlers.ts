...
export const SUPPORTED_LANGUAGES = ['auto', 'en', 'zh-CN', 'ru', 'fr', 'pl'] as const;
...
export function registerSettingsHandlers(): void {
  ...
  handle('daemon:get-socket-path', async () => {
    const { getSocketPath } = await import('@nestcafe_ai/agent-core/desktop-main');
  ...
  handle('daemon:restart', async () => {
    ...
    try {
      ...
      try {
        const { getPidFilePath } = await import('@nestcafe_ai/agent-core/desktop-main');
  ...
  // ── Memory (wiki pages) ───────────────────────────────────────────────────
  handle('memory:list-pages', async () => {
    return getDaemonClient().call('memory.listPages');
  });

  handle('memory:read-page', async (_event: IpcMainInvokeEvent, page: string) => {
    return getDaemonClient().call('memory.readPage', { page });
  });

  handle('memory:write-page', async (_event: IpcMainInvokeEvent, page: string, content: string) => {
    return getDaemonClient().call('memory.writePage', { page, content });
  });
