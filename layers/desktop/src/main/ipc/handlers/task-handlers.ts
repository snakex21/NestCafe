...
import { bootstrapDaemon, getDaemonClient, shutdownDaemon } from '../../daemon-bootstrap';
import { enableReconnect, getDataDir, suppressReconnect } from '../../daemon/daemon-connector';
...
async function restartDaemonForNewRpcMethods(): Promise<void> {
  suppressReconnect();
  try {
    try {
      getDaemonClient()
      ...
      // Daemon may already be unavailable.
    }
    shutdownDaemon();
    ...
    try {
      ...
      // Best effort; bootstrap handles stale socket retries.
    }
...
function isMissingDaemonMethodError(err: unknown): boolean {
  return String(err instanceof Error ? err.message : err).includes('Method not found');
}
  ...
  handle(
    ...
    async (
      ...
      const payload = {
        ...
        messageId: sanitizeString(messageId, 'messageId', 128),
        content: sanitizeString(content, 'content'),
        ...(originalContent !== undefined
          ? { originalContent: sanitizeString(originalContent, 'originalContent') }
          : {}),
        autoApprovePermissions,
      };
      try {
        return await getDaemonClient().call('task.rerunFromMessage', payload);
      } catch (err) {
        if (!isMissingDaemonMethodError(err)) {
          throw err;
        }
        await restartDaemonForNewRpcMethods();
        return await getDaemonClient().call('task.rerunFromMessage', payload);
      }
    },
  );
