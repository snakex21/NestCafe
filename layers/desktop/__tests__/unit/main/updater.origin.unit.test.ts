...
describe('isSameApex', () => {
  it('accepts same-apex subdomain (typical accomplish layout)', () => {
    // Feed lives on d.nestcafe.ai; artifacts on downloads.nestcafe.ai — shared apex.
    expect(isSameApex('https://downloads.nestcafe.ai/x/a.exe', 'https://d.nestcafe.ai')).toBe(true);
  });

  it('accepts identical host', () => {
    expect(isSameApex('https://d.nestcafe.ai/x/a.exe', 'https://d.nestcafe.ai')).toBe(true);
  });

  it('accepts bare apex (no subdomain)', () => {
    expect(isSameApex('https://nestcafe.ai/x/a.exe', 'https://d.nestcafe.ai')).toBe(true);
  });

  it('rejects different apex (attacker-controlled)', () => {
    expect(isSameApex('https://evil.example.com/malware.exe', 'https://d.nestcafe.ai')).toBe(false);
  });

  it('rejects label-trick hosts (myaccomplish.ai vs nestcafe.ai)', () => {
    // `myaccomplish.ai`.endsWith('nestcafe.ai') is true by raw string match, but our
    // boundary-aware check (hostname === apex || hostname.endsWith('.' + apex)) rejects it.
    expect(isSameApex('https://myaccomplish.ai/x/a.exe', 'https://d.nestcafe.ai')).toBe(false);
  });
  ...
  it('rejects IP candidate against DNS feed (or vice versa)', () => {
    expect(isSameApex('http://127.0.0.1/a.exe', 'https://d.nestcafe.ai')).toBe(false);
    expect(isSameApex('https://d.nestcafe.ai/a.exe', 'http://127.0.0.1')).toBe(false);
  });

  it('rejects HTTPS → HTTP downgrade even on same apex', () => {
    // An HTTPS feed must never direct users to a plaintext download URL.
    expect(isSameApex('http://downloads.nestcafe.ai/a.exe', 'https://d.nestcafe.ai')).toBe(false);
  });
  ...
  it('accepts same apex + same scheme (HTTPS)', () => {
    expect(isSameApex('https://downloads.nestcafe.ai/a.exe', 'https://d.nestcafe.ai')).toBe(true);
  });

  it('rejects malformed URL candidates (returns false, never throws)', () => {
    expect(isSameApex('not a url', 'https://d.nestcafe.ai')).toBe(false);
    expect(isSameApex('', 'https://d.nestcafe.ai')).toBe(false);
  });

  it('rejects when reference URL is malformed', () => {
    expect(isSameApex('https://d.nestcafe.ai/a.exe', 'not a url')).toBe(false);
  });

  it('accepts deep subdomain chains', () => {
    expect(isSameApex('https://a.b.c.nestcafe.ai/x/a.exe', 'https://d.nestcafe.ai')).toBe(true);
  });
});

describe('isTrustedManifestPath', () => {
  it('accepts relative manifest paths', () => {
    expect(isTrustedManifestPath('downloads/1.0.0/app.zip', 'https://downloads.nestcafe.ai')).toBe(
      true,
    );
  });
  ...
  it('rejects protocol-relative URLs', () => {
    expect(
      isTrustedManifestPath('//evil.example.com/app.zip', 'https://downloads.nestcafe.ai'),
    ).toBe(false);
  });

  it('rejects non-http absolute URLs', () => {
    expect(isTrustedManifestPath('file:///tmp/app.zip', 'https://downloads.nestcafe.ai')).toBe(
      false,
    );
  });
});
...
describe('isTrustedUpdateInfo', () => {
  it('accepts native update info with same-apex file URLs', () => {
    expect(
      isTrustedUpdateInfo(
        {
          files: [{ url: 'https://downloads.nestcafe.ai/downloads/1.0.0/app.zip' }],
          path: 'https://downloads.nestcafe.ai/downloads/1.0.0/app.zip',
        },
        'https://downloads.nestcafe.ai',
      ),
    ).toBe(true);
  });
