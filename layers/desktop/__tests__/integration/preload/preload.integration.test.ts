...
let capturedNestCafeAPI: Record<string, unknown> = {};
let capturednestcafeShell: Record<string, unknown> = {};

describe('Preload Script Integration', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    capturedNestCafeAPI = {};
    capturednestcafeShell = {};
    ...
    mockExposeInMainWorld.mockImplementation((name: string, api: unknown) => {
      if (name === 'accomplish') {
        capturedNestCafeAPI = api as Record<string, unknown>;
      } else if (name === 'nestcafeShell') {
        capturednestcafeShell = api as Record<string, unknown>;
      }
    });
  ...
  describe('API Exposure', () => {
    it('should expose accomplish API via contextBridge', () => {
      ...
      expect(capturedNestCafeAPI).toBeDefined();
    });

    it('should expose nestcafeShell info via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('nestcafeShell', expect.any(Object));
      expect(capturednestcafeShell).toBeDefined();
    });

    it('should expose shell info with isElectron=true', () => {
      expect(capturednestcafeShell.isElectron).toBe(true);
    });

    it('should expose shell info with platform', () => {
      expect(capturednestcafeShell.platform).toBe(process.platform);
    });

    it('should expose shell info with version matching package.json', () => {
      expect(capturednestcafeShell.version).toBe(pkg.version);
    });
  });
  ...
  describe('IPC Method Invocations', () => {
    describe('App Info', () => {
      it('getVersion should invoke app:version', async () => {
        await (capturedNestCafeAPI.getVersion as () => Promise<string>)();
        expect(mockInvoke).toHaveBeenCalledWith('app:version');
      });

      it('getPlatform should invoke app:platform', async () => {
        await (capturedNestCafeAPI.getPlatform as () => Promise<string>)();
    ...
    describe('Shell Operations', () => {
      it('openExternal should invoke shell:open-external with URL', async () => {
        ...
        await (capturedNestCafeAPI.openExternal as (url: string) => Promise<void>)(url);
    ...
    describe('Task Operations', () => {
      it('startTask should invoke task:start with config', async () => {
        ...
        await (
          capturedNestCafeAPI.startTask as (config: { description: string }) => Promise<unknown>
        )(config);
        expect(mockInvoke).toHaveBeenCalledWith('task:start', config);
      });

      it('cancelTask should invoke task:cancel with taskId', async () => {
        await (capturedNestCafeAPI.cancelTask as (taskId: string) => Promise<void>)('task_123');
    ...
    describe('Permission Operations', () => {
      it('respondToPermission should invoke permission:respond', async () => {
        ...
        await (
          capturedNestCafeAPI.respondToPermission as (r: {
            taskId: string;
            allowed: boolean;
          }) => Promise<void>
        )(response);
    ...
    describe('Session Operations', () => {
      it('resumeSession should invoke session:resume', async () => {
        await (
          capturedNestCafeAPI.resumeSession as (
            s: string,
            p: string,
            t?: string,
            attachments?: unknown[],
    ...
    describe('Settings Operations', () => {
      it('getDebugMode should invoke settings:debug-mode', async () => {
        await (capturedNestCafeAPI.getDebugMode as () => Promise<boolean>)();
        expect(mockInvoke).toHaveBeenCalledWith('settings:debug-mode');
      });

      it('setDebugMode should invoke settings:set-debug-mode', async () => {
        await (capturedNestCafeAPI.setDebugMode as (enabled: boolean) => Promise<void>)(true);
      ...
      it('getAppSettings should invoke settings:app-settings', async () => {
        await (capturedNestCafeAPI.getAppSettings as () => Promise<unknown>)();
      ...
      it('getSlackMcpOauthStatus should invoke opencode:auth:slack:status', async () => {
        await (capturedNestCafeAPI.getSlackMcpOauthStatus as () => Promise<unknown>)();
      ...
      it('loginSlackMcp should invoke opencode:auth:slack:login', async () => {
        await (capturedNestCafeAPI.loginSlackMcp as () => Promise<unknown>)();
      ...
      it('logoutSlackMcp should invoke opencode:auth:slack:logout', async () => {
        await (capturedNestCafeAPI.logoutSlackMcp as () => Promise<void>)();
    ...
    describe('Onboarding Operations', () => {
      it('getOnboardingComplete should invoke onboarding:complete', async () => {
        await (capturedNestCafeAPI.getOnboardingComplete as () => Promise<boolean>)();
      ...
      it('setOnboardingComplete should invoke onboarding:set-complete', async () => {
        await (capturedNestCafeAPI.setOnboardingComplete as (c: boolean) => Promise<void>)(true);
    ...
    describe('Model Operations', () => {
      it('getSelectedModel should invoke model:get', async () => {
        await (capturedNestCafeAPI.getSelectedModel as () => Promise<unknown>)();
      ...
      it('setSelectedModel should invoke model:set', async () => {
        ...
        await (
          capturedNestCafeAPI.setSelectedModel as (m: {
            provider: string;
            model: string;
          }) => Promise<void>
        )(model);
    ...
    describe('Logging Operations', () => {
      it('logEvent should invoke log:event', async () => {
        ...
        await (capturedNestCafeAPI.logEvent as (p: unknown) => Promise<unknown>)(payload);
    ...
    describe('Skills Operations', () => {
      it('getUserSkillsPath should invoke skills:get-user-skills-path', async () => {
        await (capturedNestCafeAPI.getUserSkillsPath as () => Promise<string>)();
  ...
  describe('Event Subscriptions', () => {
    it('onTaskUpdate should subscribe to task:update', () => {
      ...
      (capturedNestCafeAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskUpdate should return unsubscribe function', () => {
      const callback = vi.fn();
      const unsubscribe = (
        capturedNestCafeAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void
      )(callback);
      unsubscribe();
    ...
    it('onTaskUpdateBatch should subscribe to task:update:batch', () => {
      const callback = vi.fn();
      (capturedNestCafeAPI.onTaskUpdateBatch as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onPermissionRequest should subscribe to permission:request', () => {
      const callback = vi.fn();
      (capturedNestCafeAPI.onPermissionRequest as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onTaskProgress should subscribe to task:progress', () => {
      const callback = vi.fn();
      (capturedNestCafeAPI.onTaskProgress as (cb: (e: unknown) => void) => () => void)(callback);
      expect(mockOn).toHaveBeenCalledWith('task:progress', expect.any(Function));
    });

    it('onDebugLog should subscribe to debug:log', () => {
      const callback = vi.fn();
      (capturedNestCafeAPI.onDebugLog as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskStatusChange should subscribe to task:status-change', () => {
      const callback = vi.fn();
      (capturedNestCafeAPI.onTaskStatusChange as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
