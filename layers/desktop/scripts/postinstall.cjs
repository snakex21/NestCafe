...
if (process.env.NESTCAFE_POSTINSTALL_RUNNING) {
  console.log('> Postinstall already running, skipping nested invocation');
  process.exit(0);
}
process.env.NESTCAFE_POSTINSTALL_RUNNING = '1';
...
function runCommand(command, description) {
  ...
  try {
    execSync(command, {
      ...
      env: {
        ...process.env,
        NESTCAFE_POSTINSTALL_RUNNING: '1',
      },
    });
  } catch (_error) {
...
const useBundledMcp = process.env.NESTCAFE_BUNDLED_MCP === '1' || process.env.CI === 'true';
