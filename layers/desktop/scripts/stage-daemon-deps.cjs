 ...
 *   - Matches what the private nestcafe-release workflow does for
 ...
 *
 * Prerequisites:
 *   - `pnpm -F @nestcafe/desktop download:nodejs` has been run
 *     (or the build script has chained it in)
 *   - `pnpm -F @nestcafe/daemon build` has produced dist/index.js
 *
 * Usage:
 *   node apps/desktop/scripts/stage-daemon-deps.cjs [--target-platform=<p>-<a>]
 *   (typically invoked via `pnpm -F @nestcafe/desktop stage:daemon-deps`)
 */
  ...
  if (!fs.existsSync(platformRoot)) {
    die(
      `Bundled Node dir not found for host ${host}: ${platformRoot}. ` +
        `Run \`pnpm -F @nestcafe/desktop download:nodejs\` first.`,
    );
  }
  ...
  if (!fs.existsSync(nodeDir)) {
    die(
      `Expected bundled Node directory not found: ${nodeDir}. ` +
        `node-version.cjs pins v${NODE_VERSION}; run \`pnpm -F @nestcafe/desktop download:nodejs\` ` +
...
function main() {
  if (!fs.existsSync(DAEMON_DIST)) {
    die(
      `Daemon dist not found at ${DAEMON_DIST}. ` + `Run \`pnpm -F @nestcafe/daemon build\` first.`,
    );
  }
