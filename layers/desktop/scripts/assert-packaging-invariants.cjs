 ...
 *
 * Intended call sites:
 *   - OSS: `pnpm -F @nestcafe/desktop verify:package --app-root <...>`
 *     after `pnpm package:mac|win|linux`.
 *   - Free (sibling `nestcafe-release` workflow): one step per build
 *     job, invoking this script via the `commit_sha` checkout — no
 *     private copy to drift.
 *
 * On macOS Resources dir: `<dist>/mac*\/nestcafe.app/Contents/Resources`
