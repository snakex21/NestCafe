 ...
 * `@nestcafe_ai/agent-core` (`common/types/daemon.ts`) so both daemon and
...
import type { StorageAPI, ConnectedProvider, HuggingFaceLocalConfig, ProviderId, SettingsChangePayload, SettingsSnapshot } from '@nestcafe_ai/agent-core';
import type { FolderIndexingConfig, SandboxConfig } from '@nestcafe_ai/agent-core';
import type { SelectedModel, OllamaConfig, LiteLLMConfig, AzureFoundryConfig, LMStudioConfig, NimConfig } from '@nestcafe_ai/agent-core';
import type { ThemePreference, LanguagePreference } from '@nestcafe_ai/agent-core';
import type { CreditUsage } from '@nestcafe_ai/agent-core';
import type { ProviderSettings } from '@nestcafe_ai/agent-core';
...
export class SettingsService extends EventEmitter {
  ...
  getNestCafeAiCredits(): CreditUsage | null {
    return this.storage.getNestCafeAiCredits();
  }

  saveNestcafeAiCredits(usage: CreditUsage): void {
    this.storage.saveNestcafeAiCredits(usage);
    this.emit('settings.changed', { key: 'providerSettings' });
  }
  ...
  getFolderIndexingConfig(): FolderIndexingConfig {
    return this.storage.getFolderIndexingConfig();
  }
