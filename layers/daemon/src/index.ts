...
import { DaemonRpcServer, acquirePidLock } from '@nestcafe_ai/agent-core';
import {
  getSocketPath,
  getPidFilePath,
...
import { registerRpcMethods, registerTaskEventForwarding, safeHandler } from './routes/index.js';
...
// consume them yet.
import { SecretsService, SettingsService, WorkspaceService, ConnectorService, LegacyImportService } from './storage/index.js';
...
async function main(): Promise<void> {
  ...
  let NestcafeRuntime: NestcafeRuntime = noopRuntime;
  try {
    const mod = await import('@nestcafe/llm-gateway-client');
    NestcafeRuntime = mod.createRuntime();
  } catch (err: unknown) {
    const isTargetPackageMissing =
      ...
      String(err).includes("Cannot find package '@nestcafe/llm-gateway-client'");
    if (isTargetPackageMissing) {
      console.log('[Daemon] @nestcafe/llm-gateway-client not installed — OSS mode');
    } else {
      throw err;
    }
  }
  ...
  const isDevMode = process.env.NESTCAFE_DAEMON_DEV === '1';
  if (!dataDir && !isDevMode) {
    log.error(
      '[Daemon] Error: --data-dir is required.\n' +
        ...
        'For local development without --data-dir, set NESTCAFE_DAEMON_DEV=1\n' +
        'to fall back to ~/.nestcafe.',
    );
    process.exit(1);
  }

  if (!dataDir && isDevMode) {
    log.warn('[Daemon] Warning: running in dev mode without --data-dir, using ~/.nestcafe');
  }

  log.info(`[Daemon] Starting... (dataDir=${dataDir ?? '~/.nestcafe(dev fallback)'})`);
  ...
  const userDataPath = dataDir || path.join(homedir(), '.nestcafe');
  const isPackaged = args.isPackaged || process.env.NESTCAFE_IS_PACKAGED === '1';
  const resourcesPath = args.resourcesPath || process.env.NESTCAFE_RESOURCES_PATH || '';
  const appPath = args.appPath || process.env.NESTCAFE_APP_PATH || '';
  ...
  // not installed. Mirrors the `NestcafeRuntime` bootstrap pattern at
  // the top of this function — distinguishes "not installed" (silent)
  // from "installed but broken" (logs).
  const OPTIONAL_RUNTIME_MODULE = '@nestcafe/llm-gateway-client';
  ...
  const bundledSkillsCandidates = isPackaged
    ? [path.join(resourcesPath, 'bundled-skills')]
    : [
        ...(appPath
          ? [
              path.join(appPath, 'bundled-skills'),
              path.join(appPath, 'apps', 'desktop', 'bundled-skills'),
            ]
          : []),
        path.resolve(__dirname, '..', '..', '..', 'apps', 'desktop', 'bundled-skills'),
        path.resolve(process.cwd(), 'apps', 'desktop', 'bundled-skills'),
      ];
  const bundledSkillsPath =
    bundledSkillsCandidates.find((candidate) => fs.existsSync(candidate)) ??
    bundledSkillsCandidates[0];
  ...
  process.env.NESTCAFE_DAEMON_AUTH_TOKEN = authToken;
  const whatsappPort = whatsappSendApi.getPort();
  if (whatsappPort) {
    process.env.NESTCAFE_WHATSAPP_API_PORT = String(whatsappPort);
  }
