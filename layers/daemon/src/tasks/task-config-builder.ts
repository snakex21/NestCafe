...
import { getDatabase } from '@nestcafe_ai/agent-core/storage/database';
...
export async function onBeforeStart(
   ...
   * `agent.nestcafe.prompt` in the generated config file) so the
  ...
  const whatsappApiPort = getPort('NESTCAFE_WHATSAPP_API_PORT');
  ...
  const { configOptions } = await resolveTaskConfig({
    ...
    whatsappApiPort,
    authToken: process.env.NESTCAFE_DAEMON_AUTH_TOKEN,
    skills,
    workspaceId: ctx.workspaceId,
    configFileName: buildConfigFileName(ctx.taskId),
    NestcafeRuntime: opts.NestcafeRuntime,
  ...
  return {
    ...
    env,
    ...(configOptions.model ? { selectedModel: parseOpenCodeModel(configOptions.model) } : {}),
