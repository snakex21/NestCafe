...
export class TaskService extends EventEmitter {
  ...
  constructor(storage: StorageAPI, options: TaskServiceOptions) {
    ...
    this.opts = {
      ...
      NestcafeRuntime: options.NestcafeRuntime,
    };
    ...
    this.serverManager = new OpenCodeServerManager({
      ...
      NestcafeRuntime: this.opts.NestcafeRuntime,
    });
  ...
  async rerunFromMessage(params: {
    ...
    const existingTask = this.storage.getTask(params.taskId);
    if (!existingTask) {
      throw new Error(`Task not found: ${params.taskId}`);
    }

    if (
      this.taskManager.hasActiveTask(params.taskId) ||
      this.taskManager.isTaskQueued(params.taskId)
    ) {
      await this.stopTask({ taskId: params.taskId });
    }

    let messageIndex = existingTask.messages.findIndex(
      (message) => message.id === params.messageId,
    );
    if (messageIndex === -1) {
      messageIndex = findRerunFallbackMessageIndex(existingTask.messages, params.originalContent);
    }
    if (messageIndex === -1) {
      throw new Error(`Message not found: ${params.messageId}`);
    }

    const editedMessage: TaskMessage = {
      ...existingTask.messages[messageIndex],
      content: params.content,
      timestamp: new Date().toISOString(),
    };
    const messages = [...existingTask.messages.slice(0, messageIndex), editedMessage];
    const prompt = messages.find((message) => message.type === 'user')?.content ?? params.content;
    const activeModel = this.storage.getActiveProviderModel();
    const selectedModel = activeModel || this.storage.getSelectedModel();

    const task = await this._runTask(params.taskId, {
    ...
    this.storage.saveTask(rerunTask, existingTask.workspaceId);
    this.emit('statusChange', { taskId: params.taskId, status: task.status });
    return rerunTask;
  }
...
function findRerunFallbackMessageIndex(
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    if (message.type === 'user' && message.content === originalContent) {
      return index;
    }
  }
