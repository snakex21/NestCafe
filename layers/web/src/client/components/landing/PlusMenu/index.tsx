...
import type { Skill, McpConnector } from '@nestcafe_ai/agent-core/common';
...
export function PlusMenu({
  ...
  const handleRefresh = async () => {
    ...
    if (!nestcafe || isRefreshing) return;
    setIsRefreshing(true);
    try {
      const [, updatedSkills] = await Promise.all([
        new Promise((resolve) => setTimeout(resolve, 600)),
        nestcafe.resyncSkills().then(() => nestcafe.getEnabledSkills()),
      ]);
  ...
  const handleToggleConnector = useCallback(async (id: string, enabled: boolean) => {
    if (!window.nestcafe) return;
    try {
      await window.nestcafe.setConnectorEnabled(id, enabled);
