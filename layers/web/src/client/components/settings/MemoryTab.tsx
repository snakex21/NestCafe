    ...
    <div className="flex flex-col gap-3">
      ...
      <label className="flex items-center justify-between gap-4 rounded-lg border border-border bg-card px-4 py-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Po zakończeniu zadania cichy Memory Manager zapisze tylko trwałe, przydatne fakty.
            Możesz to wyłączyć.
          </p>
        </div>
        <input
              ...
              <div key={item.id} className="border-b border-border px-4 py-3 last:border-b-0">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                      item.undone ? 'bg-muted text-muted-foreground' : 'bg-primary/10 text-primary'
                    }`}
                  >
                ...
                <div className="mt-2 space-y-1">
                  {item.facts.slice(0, 3).map((fact) => (
                    <p
                      key={fact.id ?? `${fact.page}-${fact.content}`}
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-muted-foreground">
                  {page.lines} linii · {formatSize(page.size)} · aktualizacja{' '}
                  {formatDate(page.mtime)}
                </span>
              </div>
              <button
                type="button"
