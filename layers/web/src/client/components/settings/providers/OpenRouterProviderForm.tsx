...
import type { ConnectedProvider, OpenRouterCredentials } from '@nestcafe_ai/agent-core/common';
...
export function OpenRouterProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Validate key
      const validation = await nestcafe.validateApiKeyForProvider('openrouter', apiKey.trim());
      if (!validation.valid) {
      ...
      // Save key
      await nestcafe.addApiKey('openrouter', apiKey.trim());

      // Fetch models
      const result = await nestcafe.fetchOpenRouterModels();
      if (!result.success) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- debounce OpenRouter key validation and model scan
  }, [apiKey, isConnected, connecting]);
