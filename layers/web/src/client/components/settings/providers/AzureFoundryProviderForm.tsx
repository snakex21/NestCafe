...
import type { ConnectedProvider, AzureFoundryCredentials } from '@nestcafe_ai/agent-core/common';
...
export function AzureFoundryProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Validate connection
      const validation = await nestcafe.testAzureFoundryConnection({
      ...
      // Save credentials
      await nestcafe.saveAzureFoundryConfig({
        endpoint: endpoint.trim(),
        deploymentName: deploymentName.trim(),
        authType,
        apiKey: authType === 'api-key' ? apiKey.trim() : undefined,
