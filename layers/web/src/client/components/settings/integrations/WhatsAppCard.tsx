...
export function WhatsAppCard() {
  ...
  function getDisconnectLabel(): string {
    if (disconnecting) {
      return t('whatsapp.disconnecting');
    }
    if (confirmDisconnect) {
      return t('whatsapp.confirmDisconnect');
    }
    return t('whatsapp.disconnect');
  }
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30 p-4" data-testid="whatsapp-card">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-sm font-medium text-foreground">{t('whatsapp.title')}</h3>
          <p className="text-xs text-muted-foreground">{t('whatsapp.description')}</p>
        </div>
      </div>
      ...
      {isLoggedOut && (
        <div className="space-y-3">
          <div className="rounded-lg bg-destructive/10 p-3">
            <p className="text-xs text-destructive">{t('whatsapp.loggedOutMessage')}</p>
          </div>
          <button
            type="button"
          ...
          >
            {isConnecting ? t('whatsapp.connecting') : t('whatsapp.reconnect')}
          </button>
        </div>
      )}
