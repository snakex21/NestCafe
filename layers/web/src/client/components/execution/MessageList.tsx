...
function parseThinkingBlocks(content: string): ContentSegment[] {
  ...
  while ((openMatch = openRe.exec(content)) !== null) {
    if (openMatch.index > cursor) {
      segments.push({ id: nextId++, type: 'text', text: content.slice(cursor, openMatch.index) });
    }
    ...
    segments.push({ id: nextId++, type: 'think', text: content.slice(openRe.lastIndex, thinkEnd).trim() });
    cursor = afterThink;
    openRe.lastIndex = afterThink;
  }

  if (cursor < content.length) {
    segments.push({ id: nextId++, type: 'text', text: content.slice(cursor) });
  }
...
function RenderedAssistantContent({ text }: { text: string }) {
  ...
  return (
    <div className="space-y-1">
      {segments.map((seg) =>
        seg.type === 'think' ? (
          <ThinkingBlock key={seg.id} content={seg.text} />
        ) : (
          <div key={seg.id} className={proseClasses}>
            <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
              {seg.text}
            </ReactMarkdown>
          </div>
