...
export function ModelIndicator({
  ...
  const siblingModels: Array<{ id: string; displayName: string }> = (() => {
    ...
    const source =
      dynamic !== undefined
        ? dynamic.filter((m) => m.enabled !== false).map((m) => ({ id: m.id, displayName: m.name }))
  ...
  const providerLabel = activeProviderId
    ? activeProviderId.startsWith('custom:') && activeProvider?.credentials.type === 'custom'
      ? activeProvider.credentials.displayName || 'Custom Provider'
  ...
  return (
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      <DropdownMenuContent
        align="end"
        sideOffset={8}
        className="max-h-[min(420px,70vh)] w-56 overflow-y-auto overflow-x-hidden shadow-lg"
      >
        {hasModel && (
        ...
        {activeProviderId && siblingModels.length > 0 && (
          <>
            {(hasModel || otherProviders.length > 0) && <DropdownMenuSeparator />}
