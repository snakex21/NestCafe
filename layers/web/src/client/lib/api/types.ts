/**
 * NestCafe API — Type definitions for the Electron main process bridge.
 *
 * This module defines the full interface exposed by the preload script
 * via contextBridge, plus supporting types.
 */
import type {
  Task,
  TaskConfig,
...
export interface NestCafeAPI {
  ...
  // Folder Indexing
  folderIndexing: {
    scanFolders(folderPaths: string[]): Promise<
      Array<{
        path: string;
  ...
  // AI completion (text only, no image)
  aiComplete(prompt: string): Promise<{ text: string }>;

  // Modules
  listModules(): Promise<ModuleInstance[]>;
  getModule(id: string): Promise<ModuleInstance | null>;
  installModule(sourcePath: string): Promise<ModuleInstance>;
  ...
  exportBackup(options: BackupSectionOptions): Promise<BackupOperationResult>;
  importBackup(options: BackupSectionOptions): Promise<BackupOperationResult>;
  ...
  NestcafeAiEnsureReady(): Promise<{ deviceFingerprint: string }>;
  NestcafeAiDisconnect(): Promise<void>;
  NestcafeAiGetUsage(): Promise<{
    spentCredits: number;
    remainingCredits: number;
    totalCredits: number;
    resetsAt: string;
...
export interface NestcafeShell {
  version: string;
  platform: string;
  isElectron: true;
}
...
declare global {
  interface Window {
    nestcafe?: NestCafeAPI;
    nestcafeShell?: NestcafeShell;
  }
}
