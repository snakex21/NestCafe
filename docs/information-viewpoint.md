...
- SQLite DB: `~/Library/Application Support/Accomplish/nestcafe.db` (prod) / `nestcafe-dev.db` (dev)
...
| ----------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nestcafe.db`         | agent-core (SQLite) | Main database — tasks, messages, todos, providers, settings, skills, connectors. All application state.                                                                             |
| `nestcafe.db-wal`     | SQLite engine       | Write-Ahead Log — buffered writes not yet checkpointed to main DB. Auto-managed.                                                                                                    |
| `nestcafe.db-shm`     | SQLite engine       | Shared memory index for WAL. Enables concurrent readers. Auto-managed.                                                                                                              |
| `nestcafe-dev.db`     | agent-core (SQLite) | Development-mode database (used when running `pnpm dev`). Same schema as prod.                                                                                                      |
| `nestcafe-dev.db-wal` | SQLite engine       | WAL for dev database.                                                                                                                                                               |
| `nestcafe-dev.db-shm` | SQLite engine       | Shared memory for dev database.                                                                                                                                                     |
...
| Scenario                           | Look at                                                                           |
| ---------------------------------- | --------------------------------------------------------------------------------- |
| Task not completing / wrong status | `nestcafe.db` → `tasks` table (status, session_id)                              |
| Messages missing or corrupt        | `nestcafe.db` → `task_messages` table (sort_order, content)                     |
| Provider connection issues         | `nestcafe.db` → `providers` table (connection_status, credentials_type)         |
