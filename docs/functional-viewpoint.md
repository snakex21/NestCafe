...
graph TB
  ...
  subgraph BRIDGE["Preload Bridge"]
    CB["contextBridge<br/><i>window.nestcafe</i>"]
  end
...
graph TB
  subgraph BUILD["Build-time boundary"]
    ...
    BUILD_ENV["build.env<br/><i>NESTCAFE_GATEWAY_URL=...</i>"]
  end
  ...
  subgraph RUNTIME["Runtime boundary (daemon process)"]
    ...
    REAL["createRuntime()<br/><i>real NestcafeRuntime impl</i>"]
    TAG["setProxyTaskId(taskId &#124; undefined)<br/><i>hot-path callback</i>"]
  end

  subgraph USE["Consumers of NestcafeRuntime"]
    direction TB
    CG["ConfigGenerator<br/><i>buildNestcafeAiConfig(ctx)</i>"]
...
The private package's name appears on **only four lines** of OSS source. Everything else depends on the `NestcafeRuntime` _interface_ owned by agent-core.
...
### 9.2 The interface — `NestcafeRuntime` + `noopRuntime`

Defined in [`packages/agent-core/src/opencode/nestcafe-runtime.ts`](packages/agent-core/src/opencode/nestcafe-runtime.ts) and re-exported from [`agent-core`](packages/agent-core/src/index.ts):
...
### 9.4 Env-var propagation — `NESTCAFE_GATEWAY_URL`
...
getBuildConfig().accomplishGatewayUrl         [apps/desktop/.../build-config.ts:95]
    ↓
daemonEnv.NESTCAFE_GATEWAY_URL = bc.accomplishGatewayUrl
    ↓ (spawned daemon inherits env)          [apps/desktop/.../daemon-connector.ts:201]
process.env.NESTCAFE_GATEWAY_URL            (read by llm-gateway-client at createRuntime())
```
...
2. `pnpm -F @nestcafe/daemon add @accomplish/llm-gateway-client@file:/Users/…/dev/accomplish/llm-gateway-client`
3. Set `NESTCAFE_GATEWAY_URL=<dev-gateway>` in `build.env` (or inline on the `pnpm dev` command).
4. `pnpm dev` — the daemon's dynamic `import()` now resolves the local package; all four consumer paths light up.

Reverting to OSS mode is `pnpm -F @nestcafe/daemon remove @accomplish/llm-gateway-client` + restart. No other code changes required — that's the whole point of the null-object pattern.
...
| **NestcafeRuntime (interface)**    | agent-core   | Null-object (`noopRuntime`) in OSS; real impl loaded from private `@accomplish/llm-gateway-client` in Free (see §9) | `connect()`, `disconnect()`, `getUsage()`, `onUsageUpdate()`, `buildProviderConfig()`, `isAvailable()`            |
