...
main `nestcafe.db` (migration v030), and the retired `workspace-meta.db`
...
- Daemon `task-config-builder.ts` routes through `resolveTaskConfig` with
  `NestcafeRuntime`, `accomplishStorageDeps`, `configFileName`, and the
...
- `apps/daemon/src/storage-service.ts` passes `legacyMetaDbPath` into
  ...
  workspace tables live in `nestcafe.db` now.
