...
| -------------- | ------------------------------- | ------------------------------------------ |
| Database       | `<dataDir>/nestcafe.db`         | `<dataDir>\nestcafe.db`                    |
| Socket         | `<dataDir>/daemon.sock`         | `\\.\pipe\nestcafe-daemon-<hash(dataDir)>` |
...
sequenceDiagram
    ...
    React->>Preload: nestcafe.startTask({ prompt })
...
sequenceDiagram
    ...
    React->>IPC: nestcafe.resumeSession(<br/>'sess_abc123', prompt, 'tsk_001')
...
sequenceDiagram
    ...
    alt User clicks Cancel
        ...
        alt User clicks Cancel
            ...
            UI->>IPC: nestcafe.setCloseBehavior('stop-daemon')
